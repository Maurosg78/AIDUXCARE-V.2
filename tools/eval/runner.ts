...
function simulateSystem(input: any){ const s=(JSON.stringify(input||{})).toLowerCase();
...
function scoreScenario(scn: Scenario, got: any){
  ...
  if (typeof scn.expect.requiresRedaction==="boolean"){
    const ok=!!got.redacted===scn.expect.requiresRedaction; if(ok) score+=0.2; else notes.push(`redaction mismatch: got=${!!got.redacted} expect=${scn.expect.requiresRedaction}`);
  } else score+=0.2;
  if (scn.expect.code){ const ok=got.code===scn.expect.code; if(ok) score+=0.1; else notes.push(`code mismatch: got=${got.code} expect=${scn.expect.code}`); }
  const threshold=scn.expect.scoreMin??0.9; return {score, pass: score>=threshold, notes};
}
(function runAll(){
  const dir="tools/eval/scenarios"; const files=readdirSync(dir).filter(f=>f.endsWith(".json"));
  const results:Result[]=[]; for(const f of files){
    const scn:Scenario=JSON.parse(readFileSync(join(dir,f),"utf8"));
    const got=simulateSystem(scn.input); const {score,pass,notes}=scoreScenario(scn,got);
