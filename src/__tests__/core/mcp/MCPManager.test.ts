...
describe("MCPManager con Supabase", () => {
  ...
  const processingExpectation = (items: MCPMemoryBlock[]) =>
    items.map((item) => ({
      id: item.id,
  ...
  beforeEach(() => {
    ...
    vi.mocked(SupabaseDataSource.getContextualMemory).mockResolvedValue(
      mockContextualData,
    );
    vi.mocked(SupabaseDataSource.getPersistentMemory).mockResolvedValue(
      mockPersistentData,
    );
    vi.mocked(SupabaseDataSource.getSemanticMemory).mockResolvedValue(
      mockSemanticData,
    );
  });
  ...
  it("debería llamar a las 3 funciones de acceso a Supabase", async () => {
    ...
    expect(SupabaseDataSource.getContextualMemory).toHaveBeenCalledWith(
      mockVisitId,
    );
    expect(SupabaseDataSource.getPersistentMemory).toHaveBeenCalledWith(
      mockPatientId,
    );
  ...
  it("debería construir un contexto válido con datos de Supabase adaptados", async () => {
    ...
    expect(result).toEqual({
      contextual: {
        source: "supabase",
        data: processingExpectation(mockContextualData),
      },
      persistent: {
        source: "supabase",
        data: processingExpectation(mockPersistentData),
      },
      semantic: {
        source: "supabase",
        data: processingExpectation(mockSemanticData),
      },
    });
  ...
  it("debería manejar errores y devolver contexto con arrays vacíos", async () => {
    ...
    vi.mocked(SupabaseDataSource.getContextualMemory).mockRejectedValue(
  ...
  it("debería manejar valores undefined desde las funciones de Supabase", async () => {
    ...
    vi.mocked(SupabaseDataSource.getPersistentMemory).mockResolvedValue(
      undefined as unknown as MCPMemoryBlock[],
    );
    ...
    expect(result.contextual.data).toEqual(
      processingExpectation(mockContextualData),
    );
    expect(result.semantic.data).toEqual(
      processingExpectation(mockSemanticData),
    );
  });
});
