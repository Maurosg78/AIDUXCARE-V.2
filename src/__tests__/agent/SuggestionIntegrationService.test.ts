...
describe('SuggestionIntegrationService', () => {
  ...
  describe('integrateSuggestion', () => {
    it('debe integrar una sugerencia en un campo vacío', async () => {
      ...
      // Verificar que se llamó a upsert con el contenido correcto
      expect(supabase.from).toHaveBeenCalledWith('emr_fields');
      expect(supabase.from('emr_fields').upsert).toHaveBeenCalledWith({
        visit_id: mockVisitId,
        field_name: mockSuggestion.field,
    ...
    it('debe agregar una sugerencia a un campo existente con prefijo', async () => {
      ...
      // Verificar que se llamó a upsert con el contenido concatenado
      expect(supabase.from('emr_fields').upsert).toHaveBeenCalledWith({
        visit_id: mockVisitId,
        field_name: mockSuggestion.field,
