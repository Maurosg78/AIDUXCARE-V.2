...
export class ClinicalInsightsEngine {
  ...
  static async generateClinicalInsights(
    ...
    try {
      ...
      const patterns = await this.detectClinicalPatterns(
      ...
      const insights: ClinicalInsightSummary = {
        ...
        processing_metadata: {
          insights_generated:
            patterns.length + alerts.length + recommendations.length,
      ...
      const enrichedInsights =
        await this.enrichWithScientificEvidence(insights);
      ...
      console.log(
        `✅ Insights generados: ${insights.processing_metadata.insights_generated} total`,
      );
        ...
        alerts: [
          {
            ...
            description:
              "No se pudieron generar insights avanzados para esta sesión",
  ...
  private static async detectClinicalPatterns(
    ...
    if (
      soapNotes.assessment.toLowerCase().includes("mejora") ||
  ...
  private static async generateClinicalAlerts(
    ...
    const fullText =
      `${soapNotes.subjective} ${soapNotes.objective} ${soapNotes.assessment}`.toLowerCase();
    const detectedRedFlags = redFlagKeywords.filter((flag) =>
      fullText.includes(flag),
    );
    ...
    if (detectedRedFlags.length > 0) {
      alerts.push({
        ...
        rationale:
          "Los red flags requieren evaluación médica inmediata para descartar patología grave",
        evidence_based: true,
        immediate_actions: [
    ...
    const incompleteSections = soapSections.filter(
    ...
    if (incompleteSections.length > 1) {
      alerts.push({
        ...
        rationale:
          "La documentación completa es esencial para continuidad de cuidado y aspectos legales",
        evidence_based: true,
        immediate_actions: [
    ...
    if (entities.length > 8) {
      alerts.push({
        ...
        rationale:
          "Sesiones complejas pueden requerir tiempo adicional y seguimiento especial",
        evidence_based: false,
        immediate_actions: [
  ...
  private static async generateProactiveRecommendations(
    ...
    if (progressPattern) {
      recommendations.push({
        ...
        description:
          "El paciente muestra signos positivos de mejora - considerar intensificación terapéutica",
        clinical_justification:
          "Los indicadores de progreso sugieren que el paciente puede tolerar mayor intensidad",
        expected_outcomes: [
    ...
    if (riskEntities.length > 0) {
      recommendations.push({
        ...
        clinical_justification:
          "La modificación de factores de riesgo previene recurrencias y mejora outcomes a largo plazo",
        expected_outcomes: [
    ...
    const fullText =
      `${soapNotes.subjective} ${soapNotes.objective} ${soapNotes.assessment} ${soapNotes.plan}`.toLowerCase();
    ...
    if (treatmentEntities.length >= 2 && !fullText.includes("tecnología")) {
      recommendations.push({
        ...
        description:
          "Considerar tecnologías complementarias para potenciar tratamiento actual",
        clinical_justification:
          "La integración tecnológica puede mejorar precision y adherencia al tratamiento",
        expected_outcomes: [
  ...
  private static async assessOverallClinicalStatus(
    ...
    complexityScore +=
      alerts.filter((a) => a.severity === "critical").length * 40;
    complexityScore +=
      alerts.filter((a) => a.severity === "warning").length * 20;
    ...
    const intervention_urgency =
      criticalAlerts.length > 0
        ? "immediate"
        : alerts.filter((a) => a.severity === "warning").length > 1
          ? "urgent"
    ...
    const prognosis_indicator =
      progressPatterns.length > 0 && criticalAlerts.length === 0
        ? "excellent"
        : positiveRecommendations.length > 0 && alerts.length <= 1
          ? "good"
          : alerts.length > 2 || criticalAlerts.length > 0
            ? "guarded"
            : "good";
    ...
    qualityScore +=
      recommendations.filter((r) => r.type === "optimization").length * 10;
  ...
  private static async enrichWithScientificEvidence(
    ...
    try {
      ...
      for (const pattern of insights.patterns.filter(
        ...
        try {
          ...
          if (ragResult.citations.length > 0) {
            pattern.evidence_support.scientific_articles =
  ...
  private static calculateOverallConfidence(
    ...
    const alertConfidence =
      alerts.length > 0 ? evidenceBasedAlerts / alerts.length : 0.5;

    const strongRecommendations = recommendations.filter(
      (r) => r.evidence_support.strength_of_recommendation === "strong",
    ).length;
    const recConfidence =
      recommendations.length > 0
        ? (strongRecommendations * 0.9 +
            (recommendations.length - strongRecommendations) * 0.6) /
