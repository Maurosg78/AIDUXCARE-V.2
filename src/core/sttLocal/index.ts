...
export function detectSTTCapabilities(): STTLocalCapabilities {
  ...
  const webGPUSupported = typeof (navigator as Record<string, unknown>).gpu !== 'undefined';
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        const audioInfo = await (sttLocalEngine as { validateAudio: (blob: Blob) => Promise<{ duration: number; sampleRate: number; channels: number }> }).validateAudio(audioBlob);
        
        const transcription: LocalTranscription = {
          id: crypto.randomUUID(),
          audioBlob: result.fallback ? undefined : audioBlob, // Solo guardar audio si no es fallback
          text: result.text,
