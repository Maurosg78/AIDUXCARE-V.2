...
export class PubMedSearchService {
  ...
  static async searchArticles(
    ...
    try {
        ...
        if (article) {
          try {
            documents.push({
              ...
              evidence_level: this.determineEvidenceLevel(
                article.title,
                article.pubtype,
              ),
      ...
      console.log(
        `✅ Encontrados ${documents.length} artículos en PubMed para: ${query}`,
      );
      return documents;
    } catch (error) {
   ...
   */
  private static buildSpecializedQuery(
    query: string,
    specialty: MedicalSpecialty,
  ): string {
   ...
   */
  private static determineEvidenceLevel(
    title: string,
    _pubTypes: string[] = [],
  ): EvidenceLevel {
...
export class RAGMedicalMCP {
  ...
  static async retrieveRelevantKnowledge(
    ...
    try {
      ...
      // 1. Buscar en PubMed
      const documents = await PubMedSearchService.searchArticles(
        clinicalQuery,
        specialty,
        maxResults,
      );
      ...
      if (documents.length === 0) {
        return {
          ...
          relevant_chunks: [],
          medical_context:
            "No se encontró evidencia científica específica para esta consulta.",
      ...
      console.log(
        `✅ RAG completado: ${documents.length} documentos, ${allChunks.length} chunks`,
      );
      ...
      return {
        ...
        relevant_chunks: [],
        medical_context:
          "Error al acceder a las fuentes de conocimiento médico.",
        confidence_score: 0.0,
        citations: [],
        processing_time_ms: Date.now() - startTime,
      };
...
TIEMPO DE PROCESAMIENTO: ${ragResult.processing_time_ms}ms`,
    ...
    ragResult.relevant_chunks.slice(0, 3).forEach((chunk, index) => {
      const citation = ragResult.citations.find(
...
RELEVANCIA: ${Math.round((citation?.relevance_score || 0) * 100)}%`,
  ...
  private static buildMedicalContext(
    ...
    const abstractChunks = chunks.filter(
      (c) => c.metadata.section_type === "abstract",
    );
...
RECOMENDACIÓN: Considerar esta evidencia en el contexto clínico específico del paciente.`;
  ...
  private static calculateOverallConfidence(
    ...
    const avgRelevance =
      docs.reduce(
        (sum, doc) => sum + this.calculateRelevanceScore(doc, query),
        0,
      ) / docs.length;

    const evidenceBonus = docs.some(
      (d) => d.evidence_level === "level_1" || d.evidence_level === "level_2",
    )
      ? 0.1
      : 0;
...
export class RAGTestingHelper {
  ...
  static async testRAGPipeline(): Promise<void> {
    ...
    for (const query of testQueries) {
      ...
      const result = await RAGMedicalMCP.retrieveRelevantKnowledge(
        query,
        "fisioterapia",
        3,
      );
