...
const AuditLogDemo: React.FC = () => {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Demo de Auditoría MCP</h1>
      ...
      <div className="mb-10 p-4 bg-gray-50 rounded border">
        ...
        <ul className="list-disc pl-5 mb-4">
          ...
          <li>Logs para visita <code>{testVisitId}</code>: {allLogs.filter(log => log.visit_id === testVisitId).length}</li>
          <li>Logs para visita <code>{otherVisitId}</code>: {allLogs.filter(log => log.visit_id === otherVisitId).length}</li>
        </ul>
      </div>
