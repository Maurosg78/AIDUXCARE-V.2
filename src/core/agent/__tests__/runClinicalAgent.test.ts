...
describe("runClinicalAgent", () => {
  beforeEach(() => {
    ...
    vi.mocked(AgentExecutor.create).mockResolvedValue(
      mockExecutorInstance as unknown as AgentExecutor,
    );
  });
  ...
  it("debe manejar errores en el proceso de ejecución del agente", async () => {
    ...
    const mockErrorExecutor = {
      execute: vi
        .fn()
        .mockRejectedValue(new Error("Error durante la ejecución")),
      getSuggestions: vi.fn().mockReturnValue([]),
    };

    vi.mocked(AgentExecutor.create).mockResolvedValueOnce(
      mockErrorExecutor as unknown as AgentExecutor,
    );
