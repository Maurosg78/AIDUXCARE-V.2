...
export class SuggestionIntegrationService {
  ...
  public static async integrateSuggestion(
    ...
    try {
      ...
      const { error } = await supabase.from("integrated_suggestions").insert({
      ...
      if (error) {
        ...
        throw new Error(
          `Error al registrar la integración de la sugerencia: ${error.message}`,
        );
      }
      ...
      const { error: upsertError } = await supabase.from("emr_fields").upsert({
        visit_id: visitId,
      ...
      if (upsertError) {
        ...
        throw new Error(
          `Error al integrar la sugerencia: ${upsertError.message}`,
        );
      }
