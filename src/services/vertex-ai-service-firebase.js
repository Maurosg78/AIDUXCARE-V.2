...
const extractTextField = (data) => {
    ...
    if (typeof data.answer === 'string')
        return data.answer;
    if (typeof data.answerText === 'string')
...
const buildPreExtractedMedicalHistoryContext = (items) => {
    if (items.length === 0) {
        return '';
    }
...
const buildVoiceSummaryPrompt = (transcript, language) => {
...
const buildVoiceClinicalInfoPrompt = (params) => {
...
export async function analyzeWithVertexProxy(payload) {
    ...
    let identifiersMap = {};
    let preExtractedMajorMedicalHistory = [];
    if (payload.transcript && !payload.prompt) {
        ...
        preExtractedMajorMedicalHistory = await extractMajorMedicalHistory(deidentifiedText, async (prompt) => {
...
export class VertexAIServiceViaFirebase {
    static async processWithNiagara(payload) {
    ...
    static async runVoiceSummary(params) {
    ...
    static async runVoiceClinicalInfo(params) {
