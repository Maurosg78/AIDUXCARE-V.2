...
export class AzureOpenAIService {
  ...
  constructor(config?: Partial<AzureOpenAIConfig>) {
    this.config = {
      endpoint: process.env.VITE_AZURE_OPENAI_ENDPOINT || 'https://openai-aiduxcare-mvp.openai.azure.com/',
  ...
  async checkAvailability(): Promise<boolean> {
    ...
    try {
      const response = await fetch(`${this.config.endpoint}/openai/deployments?api-version=${this.config.apiVersion}`, {
        method: 'GET',
        headers: {
  ...
  async sendRequest(
    ...
    if (!isAvailable || this.quotaExceeded) {
      if (options?.useFallback !== false) {
        return this.useFallback(messages, options);
      }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.status === 429 || errorData.error?.code === 'InsufficientQuota') {
