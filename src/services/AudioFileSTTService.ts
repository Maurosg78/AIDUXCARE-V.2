...
export class AudioFileSTTService {
  ...
  private async validateAudioFile(file: File): Promise<void> {
    ...
    // Verificar tipo MIME
    const isValidType = this.supportedMimeTypes.some(
      (type) =>
        file.type === type ||
    ...
    if (!isValidType) {
      throw new Error(
        `Formato de archivo no soportado. Use: ${this.supportedMimeTypes.join(", ")}`,
      );
    }
  }
  ...
  private async getAudioInfo(
    file: File,
  ): Promise<AudioFileSTTResult["audioInfo"]> {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      try {
        ...
        const adjustedSegments = chunkSegments.map((segment) => ({
          ...segment,
          timestamp: new Date(
            new Date(segment.timestamp).getTime() + startTime * 1000,
          ).toISOString(),
        }));
  ...
  private async processAudioComplete(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.playAudioForTranscription(
        file,
        sttService,
  ...
  private generateSimulatedTranscription(
    ...
    const segments: TranscriptionSegment[] = [];
    const _wordsPerMinute = 120; // Velocidad promedio de habla
    ...
    for (let i = 0; i < totalSegments; i++) {
      const timestamp = new Date(
        Date.now() + i * segmentDuration * 1000,
      ).toISOString();
      ...
      segments.push({
        ...
        actor:
          content.toLowerCase().includes("fisio") ||
          content.toLowerCase().includes("physio")
            ? "profesional"
            : "paciente",
        approved: false,
        edited: false,
  ...
  private calculateQualityMetrics(
  ...
  ): AudioFileSTTResult["qualityMetrics"] {
    const confidenceValues = segments.map((s) =>
      this.confidenceToNumber(s.confidence),
    );
    ...
    return {
      ...
      wordsPerMinute: Math.round(_wordsPerMinute),
      detectedSpeakers,
    };
  }
