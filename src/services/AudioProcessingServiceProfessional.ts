...
export class AudioProcessingServiceProfessional {
    ...
    try {
      ...
      if (config.enableClinicalInsights) {
        try {
          ...
          clinicalInsights =
            await ClinicalInsightsEngine.generateClinicalInsights({
          ...
          console.log(
            `✅ Insights generados: ${clinicalInsights.processing_metadata.insights_generated} total`,
          );
        } catch (insightsError) {
      ...
      AuditLogger.log("audio.processing.completed", {
        ...
        insightsGenerated:
          clinicalInsights?.processing_metadata.insights_generated || 0,
        clinicalComplexity:
          clinicalInsights?.overall_assessment.clinical_complexity || "unknown",
        interventionUrgency:
          clinicalInsights?.overall_assessment.intervention_urgency ||
          "unknown",
      });
      ...
      trackMetric(
        ...
        {
          suggestionId: processingId,
          suggestionType: "recommendation" as const,
          suggestionField: "audio_processing",
        },
        userId,
        visitId,
      );
      ...
      trackMetric(
        ...
        {
          suggestionId: processingId,
          suggestionType: "warning" as const,
          suggestionField: "error",
        },
        userId,
        visitId,
      );
  ...
  private static detectActor(content: string): TranscriptionActor {
    ...
    const professionalScore = professionalKeywords.reduce(
      (score, keyword) => (lowerContent.includes(keyword) ? score + 1 : score),
      0,
    );
   ...
   */
  private static transcriptionToText(
    transcription: TranscriptionSegment[],
  ): string {
  ...
  private static async buildPhysiotherapyContext(
    ...
    const processedTranscript = {
      ...
      segments: transcription.map((seg) => ({
        ...
        timestamp_end: 0,
        confidence:
          seg.confidence === "entendido"
            ? 0.9
            : seg.confidence === "poco_claro"
              ? 0.6
              : 0.3,
      })),
      entities: nlpResult.entities,
    ...
    return {
      ...
      processing_flags: {
        ...
        contains_red_flags: this.detectRedFlags(
          nlpResult.entities,
          nlpResult.soapNotes,
        ),
        incomplete_information:
          (nlpResult.metrics.overall_confidence || 0) < 0.7,
  ...
  private static async generateAgentSuggestions(
    ...
    if (nlpResult.entities.length > 0) {
      const symptomEntities = nlpResult.entities.filter(
        (e) => e.type === "symptom",
      );
      if (symptomEntities.length > 0) {
        suggestions.push({
          ...
          type: "recommendation",
          content: `Se identificaron ${symptomEntities.length} síntomas principales. Considere evaluar: ${symptomEntities
            .slice(0, 3)
            .map((e) => e.text)
            .join(", ")}.`,
    ...
    if (
      nlpResult.soapNotes.confidence_score &&
      nlpResult.soapNotes.confidence_score < 0.7
    ) {
      suggestions.push({
        id: `suggestion_soap_review_${Date.now()}`,
        type: "warning",
        content:
          "La nota SOAP generada tiene baja confianza. Recomiende revisión manual para asegurar precisión clínica.",
    ...
    if (context.processing_flags?.contains_red_flags) {
      suggestions.push({
        ...
        type: "warning",
        content:
          "Se detectaron indicadores que requieren atención médica inmediata. Revise cuidadosamente antes de proceder.",
  ...
  private static async assessQuality(
    ...
    _agentSuggestions: AgentSuggestion[],
    ...
    const clinical_relevance = this.calculateClinicalRelevance(
      nlpResult.entities,
    );
    ...
    const overall_score = Math.round(
      (completeness +
        clinical_relevance +
        (nlpResult.metrics.overall_confidence
          ? nlpResult.metrics.overall_confidence * 100
          : 0)) /
        3,
    );

    // Detectar red flags
    const red_flags = this.detectRedFlagsList(
      nlpResult.entities,
      nlpResult.soapNotes,
    );
    ...
    const confidence_level =
      overall_score >= 80 ? "high" : overall_score >= 60 ? "medium" : "low";
  ...
  private static buildProfessionalMetrics(
      ...
      transcription.length > 0
        ? transcription.reduce((sum, t) => {
            ...
            const confidenceValue =
              t.confidence === "entendido"
                ? 0.9
                : t.confidence === "poco_claro"
                  ? 0.6
                  : 0.3;
    ...
    const baseMetrics = {
      ...
      entity_extraction_time_ms:
        nlpResult.metrics?.entity_extraction_time_ms || 600,
      entities_extracted: nlpResult.entities.length,
      soap_generation_time_ms:
        nlpResult.metrics?.soap_generation_time_ms || 900,
      soap_completeness: this.calculateCompleteness(
        transcription,
        nlpResult.soapNotes,
      ),
      ...
      overall_confidence:
        ...
        3,
      requires_review:
        qualityAssessment.requires_review ||
        clinicalInsights?.overall_assessment.intervention_urgency ===
          "immediate" ||
        clinicalInsights?.alerts.some((a) => a.severity === "critical") ||
        false,
    };
    ...
    if (clinicalInsights) {
      return {
        ...
        insights_processing_time_ms:
          clinicalInsights.processing_metadata.processing_time_ms,
        insights_generated:
          clinicalInsights.processing_metadata.insights_generated,
        ...
        clinical_complexity_score: this.mapComplexityToScore(
          clinicalInsights.overall_assessment.clinical_complexity,
        ),
        ai_confidence_insights:
          clinicalInsights.processing_metadata.ai_confidence,
        evidence_sources_used:
          clinicalInsights.processing_metadata.evidence_sources,
  ...
  private static calculateCompleteness(
    ...
    const recognizedSegments = transcription.filter(
      (s) => s.confidence !== "no_reconocido",
    ).length;
    ...
    const completeSoapFields = soapFields.filter(
  ...
  private static calculateClinicalRelevance(
    entities: ClinicalEntity[],
  ): number {
    ...
    const treatmentCount = entities.filter(
      (e) => e.type === "treatment",
    ).length;
    const diagnosisCount = entities.filter(
      (e) => e.type === "diagnosis",
    ).length;
  ...
  private static detectRedFlags(
    ...
    const fullText =
      `${soap.subjective} ${soap.objective} ${soap.assessment} ${soap.plan}`.toLowerCase();
    return redFlagKeywords.some((keyword) => fullText.includes(keyword));
  }

  private static detectRedFlagsList(
    entities: ClinicalEntity[],
    soap: SOAPNotes,
  ): string[] {
    ...
    const redFlagKeywords = [
      {
        keyword: "dolor severo",
        flag: "Dolor severo reportado - requiere evaluación inmediata",
      },
      { keyword: "fractura", flag: "Posible fractura - derivar a radiología" },
      {
        keyword: "pérdida de sensibilidad",
        flag: "Pérdida neurológica - evaluación urgente",
      },
    ];
  ...
  private static generateQualityRecommendations(
    ...
    if (overall_score < 70) {
      recommendations.push(
        "Revisar transcripción manualmente antes de integrar al EMR",
      );
    }

    if (completeness < 60) {
      recommendations.push(
        "Completar información faltante en la documentación clínica",
      );
    }

    if (clinical_relevance < 50) {
      recommendations.push(
        "Agregar más detalles clínicos específicos de fisioterapia",
      );
    }

    if (red_flags.length > 0) {
      recommendations.push(
        "Revisar inmediatamente los indicadores de alerta identificados",
      );
    }
