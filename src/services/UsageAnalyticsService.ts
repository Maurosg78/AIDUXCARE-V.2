 ...
 */
export const getMetricsSummaryByVisit = (
  visitId: string,
): {
  generated: number;
  accepted: number;
  ...
  return {
    ...
    field_matched: metrics
      .filter(
        (m) => m.type === "suggestions_generated" && m.metadata?.field_matched,
      )
      .reduce((sum, m) => sum + m.value, 0),
 ...
 */
export const saveLongitudinalMetricsToSupabase = async (
  ...
  try {
    ...
    const { error } = await client.from("metrics_by_visit").insert([
      {
        ...metric,
 ...
 */
export const getLongitudinalMetricsByPatient = async (
  patientId: string,
): Promise<LongitudinalMetric[]> => {
  try {
    ...
    return data.map((metric: MetricData) => ({
      ...
      risk_level_summary: metric.risk_level_summary as
        | "low"
        | "medium"
        | "high",
      clinical_evolution: metric.clinical_evolution as
        | "improved"
        | "stable"
        | "worsened",
 ...
 */
export const getLongitudinalMetricForVisit = async (
  visitId: string,
): Promise<LongitudinalMetric | null> => {
  try {
    // Usar el cliente Supabase disponible
    ...
    if (error) {
      ...
      if (error.code === "PGRST116" || error.code === "42P01") {
        console.warn(
          "Tabla metrics_by_visit no encontrada, generando métricas simuladas",
        );
        return generateFallbackMetric(visitId);
      }

      // Si no encontramos métricas, pero la tabla existe, buscar en el almacén local
      const metric = longitudinalMetricsStore.find(
        (m) => m.visit_id === visitId,
      );
...
const generateFallbackMetric = (visitId: string): LongitudinalMetric => {
  ...
  const evolutions: Array<"improved" | "stable" | "worsened"> = [
    "improved",
    "stable",
    "worsened",
  ];
  ...
  const fallbackMetric: LongitudinalMetric = {
    ...
    details: {
      ...
      pain_level_comparison: {
        previous:
          randomEvolution === "improved"
            ? 7
            : randomEvolution === "stable"
              ? 5
              : 3,
        current:
          randomEvolution === "improved"
 ...
 */
export const getEvolutionIndicator = (
  ...
  switch (evolution) {
    case "improved":
      return "🟢"; // Verde: mejora
    case "stable":
      return "🟡"; // Amarillo: estable
    case "worsened":
      return "🔴"; // Rojo: empeoramiento
    default:
      return "⚪"; // Blanco: desconocido
  }
};
