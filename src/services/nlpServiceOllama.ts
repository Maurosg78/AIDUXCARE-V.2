...
export class NLPServiceOllama {
  ...
  private static promptVersionConfig = {
    useOptimizedV2:
      localStorage?.getItem("aiduxcare_prompt_version") === "v2" || false,
    autoLogging: true,
    testingMode:
      localStorage?.getItem("aiduxcare_testing_mode") === "true" || false,
  };
...
`;
      ...
      if (jsonMatch) {
        try {
          ...
          entities = parsed.map(
            (
              entity: { type?: string; text?: string; confidence?: number },
              index: number,
            ) => ({
      ...
      if (useRAG && entities.length > 0) {
        try {
          ...
          if (keyTerms.length > 0) {
            console.log(
              "🔬 Enriqueciendo entidades con evidencia científica...",
            );
      ...
      console.log(
        `✅ Entidades extraídas: ${entities.length} en ${processingTime}ms`,
      );
  ...
  static async generateSOAPNotes(
    ...
    const startTime = Date.now();
    const promptVersion = this.promptVersionConfig.useOptimizedV2
      ? "v2"
      : "current";
    ...
    try {
      ...
      if (this.promptVersionConfig.useOptimizedV2) {
        result = await this.generateSOAPNotesOptimizedV2(
          transcript,
          entities,
          useRAG,
        );
      } else {
        result = await this.generateSOAPNotesOriginal(
          transcript,
          entities,
          useRAG,
        );
      ...
      if (
        this.promptVersionConfig.testingMode &&
        this.promptVersionConfig.autoLogging
      ) {
        const processingTime = Date.now() - startTime;
        await this.logTestingMetrics(
          promptVersion,
          processingTime,
          result,
          transcript,
   ...
   */
  private static async generateSOAPNotesOriginal(
    transcript: string,
    entities: ClinicalEntity[],
    useRAG: boolean = true,
  ): Promise<SOAPNotes> {
...
${ragResult.citations
  .slice(0, 2)
  .map((c) => `- ${c.title.substring(0, 80)}... (${c.year})`)
  .join("\n")}`;

            console.log(
              `✅ RAG: Añadida evidencia de ${ragResult.citations.length} fuentes`,
            );
          }
        }
      } catch (error) {
...
`;
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          const soapNotes: SOAPNotes = {
            subjective:
              soapData.subjective || "Información subjetiva no disponible",
            objective:
              soapData.objective || "Observaciones objetivas no registradas",
            assessment:
              soapData.assessment || "Evaluación pendiente de completar",
          ...
          console.log(
            `✅ Nota SOAP original generada en ${processingTime}ms${useRAG ? " (con evidencia)" : ""}`,
          );
          return soapNotes;
        } catch (parseError) {
      ...
      if (error instanceof Error && error.message.includes("timeout")) {
        console.log(
          "🔄 Timeout detectado en prompt original, intentando versión simplificada...",
        );
        return this.generateSimplifiedSOAP(transcript, entities);
      }

      throw new Error(
        `Failed to generate original SOAP notes: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  static async generateSOAPNotesOptimizedV2(
    transcript: string,
    entities: ClinicalEntity[],
    useRAG: boolean = true,
  ): Promise<SOAPNotes> {
...
{"subjective":"","objective":"","assessment":"","plan":""}`;
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          const soapNotes: SOAPNotes = {
            subjective:
              soapData.subjective || `Paciente reporta: ${keyInfo.symptoms}`,
            objective:
              soapData.objective || `Evaluación revela: ${keyInfo.findings}`,
            assessment:
              soapData.assessment ||
              `Análisis: ${keyInfo.primaryCondition || "condición evaluada"}`,
          ...
          console.log(
            `✅ SOAP Optimizado v2 generado en ${processingTime}ms${useRAG ? " (con evidencia)" : ""}`,
          );
          return soapNotes;
        } catch (parseError) {
      ...
      return {
        ...
        assessment:
          keyInfo.primaryCondition || "Condición evaluada según hallazgos",
      ...
      return {
        subjective:
          keyInfo.symptoms || `Paciente: ${transcript.substring(0, 100)}...`,
   ...
   */
  private static extractKeyInfoForSOAP(
    transcript: string,
    entities: ClinicalEntity[],
  ): {
    symptoms: string;
    ...
    const primaryCondition =
      entities.find((e) => e.type === "diagnosis")?.text ||
   ...
   */
  private static async generateSimplifiedSOAP(
    transcript: string,
    entities: ClinicalEntity[],
  ): Promise<SOAPNotes> {
...
{"subjective":"","objective":"","assessment":"","plan":""}`;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          assessment:
            soapData.assessment || "Condición evaluada según hallazgos",
  ...
  static async processTranscript(
    transcript: string,
    options: { useRAG?: boolean; useOptimizedSOAP?: boolean } = {},
  ): Promise<{
    entities: ClinicalEntity[];
    soapNotes: SOAPNotes;
    ...
    try {
      console.log(
        `🧠 Procesando transcripción${useRAG ? " con RAG" : " sin RAG"}${useOptimizedSOAP ? " (SOAP Optimizado v2)" : ""}...`,
      );
      ...
      if (useRAG && entities.length > 2) {
        try {
          ...
          if (keyTerms.length > 0) {
            ...
            ragResult = await RAGMedicalMCP.retrieveRelevantKnowledge(
              ragQuery,
              "fisioterapia",
              5,
            );
            ...
            console.log(
              `🔬 RAG UI: ${ragResult.citations.length} artículos para interfaz de usuario`,
            );
          }
        } catch (ragError) {
      ...
      const metrics: ProcessingMetrics = {
        ...
        estimated_tokens_used: Math.ceil(
          (transcript.length + JSON.stringify(entities).length) / 4,
        ), // Estimación
        model_used: "llama3.2:3b",
      };

      console.log(
        `✅ Procesamiento completo en ${totalTime}ms - Entidades: ${entities.length}, SOAP: ${soapNotes.confidence_score?.toFixed(2)}, RAG: ${ragResult?.citations.length || 0} citas`,
      );
   ...
   */
  private static extractEntitiesWithRegex(
    transcript: string,
  ): ClinicalEntity[] {
   ...
   */
  private static generateFallbackSOAP(
    transcript: string,
    _useRAG: boolean,
  ): SOAPNotes {
    ...
    return {
      ...
      assessment:
        "Condición evaluada conforme a hallazgos clínicos. Respuesta al tratamiento monitoreada.",
   ...
   */
  private static calculateSOAPConfidence(soapData: {
    subjective?: string;
    objective?: string;
    assessment?: string;
    plan?: string;
    ...
    const validFields = fields.filter(
      (field) => field && field.length > 10,
    ).length;
    const avgLength =
      fields.reduce((sum, field) => sum + (field?.length || 0), 0) / 4;
    ...
    return (
      Math.round((completenessScore * 0.7 + lengthScore * 0.3) * 100) / 100
    );
  }
  ...
  private static calculateOverallConfidence(
    ...
    return (
      Math.round((entitiesConfidence * 0.4 + soapConfidence * 0.6) * 100) / 100
    );
  }
  ...
  private static requiresReview(
    ...
    const hasLowConfidenceEntities = entities.some(
      (e) => (e.confidence || 0) < 0.5,
    );
    const hasShortSOAPFields = [
      soap.subjective,
      soap.objective,
      soap.assessment,
      soap.plan,
    ...
    return (
      overallConfidence < 0.7 || hasLowConfidenceEntities || hasShortSOAPFields
    );
  }
