...
describe("MCPContextViewer", () => {
  ...
  it("muestra correctamente el contenido de la memoria contextual", () => {
    ...
    expect(
      screen.getByText("Información contextual de prueba"),
    ).toBeInTheDocument();
    expect(screen.getByText("ID: ctx-1")).toBeInTheDocument();
    expect(
      screen.getByText(
        (content) =>
          content.includes("15/05/2025") && content.includes("12:30"),
      ),
    ).toBeInTheDocument();
  });
  ...
  it("muestra correctamente el contenido de la memoria persistente", () => {
    ...
    expect(
      screen.getByText("Historial médico del paciente"),
    ).toBeInTheDocument();
    expect(screen.getByText("ID: per-1")).toBeInTheDocument();
    expect(
      screen.getByText(
        (content) =>
          content.includes("10/05/2025") && content.includes("10:15"),
      ),
    ).toBeInTheDocument();
  });
  ...
  it("muestra un mensaje cuando no hay datos en una sección", () => {
    ...
    expect(
      screen.getByText("Sin datos disponibles en esta sección"),
    ).toBeInTheDocument();
  });
  ...
  describe("Modo editable", () => {
    ...
    it("muestra las instrucciones", () => {
      ...
      expect(
        screen.getByText(
          "Edite el contenido de los bloques según sea necesario",
        ),
      ).toBeInTheDocument();
    });
    ...
    it("permite editar contenido de un bloque", () => {
      ...
      fireEvent.change(textarea, {
        target: { value: "Nuevo contenido editado" },
      });
