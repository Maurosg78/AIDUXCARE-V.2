...
describe("AuditLogViewer", () => {
  ...
  it("renderiza correctamente el componente con logs locales", () => {
    render(<AuditLogViewer visitId={visitId} logs={mockLogs} />);
  ...
  it("muestra los detalles correctos para cada tipo de log", () => {
    ...
    expect(mcpLog).toHaveTextContent(
      "Contenido de prueba para bloque contextual",
    );
  ...
  it("muestra un mensaje cuando no hay logs para mostrar", () => {
    render(<AuditLogViewer visitId={visitId} logs={[]} />);
    ...
    expect(
      screen.getByText(/No hay registros de actividad para mostrar/i),
    ).toBeInTheDocument();
  });
