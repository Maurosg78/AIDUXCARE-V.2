 ...
 */
const isMCPUpdateEntry = (
  log: AuditLogEntry,
): log is AuditLogEntry & { event_type: "mcp.block.update" } => {
 ...
 */
const isSuggestionIntegrationEntry = (
  log: AuditLogEntry,
): log is AuditLogEntry & { event_type: "suggestion.integrated" } => {
      ...
      return (
        <div className="mt-2 text-sm">
          <p className="text-gray-600">
            ...
            {getBlockTypeText(details.block_type || "")}
          </p>
          <p className="text-gray-600">
            <span className="font-medium">Contenido:</span>{" "}
            {truncateContent(details.block_content || "")}
          </p>
        </div>
      );
    }
    ...
    if (isSuggestionIntegrationEntry(log)) {
      ...
      return (
        <div className="mt-2 text-sm">
          ...
          <p className="text-gray-600">
            <span className="font-medium">Contenido:</span>{" "}
            {truncateContent(details.suggestion_content || "")}
          </p>
        </div>
      );
    }
    ...
    <div className="mt-8 border rounded-md border-gray-200 bg-white shadow-sm">
      <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
        <h3 className="text-lg font-medium text-gray-800">
          Historial de Actividad Clínica{" "}
          {fromSupabase ? "(Supabase)" : "(Local)"}
        </h3>
        <button
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span
                        className={`px-2 py-1 text-xs rounded-full ${getSourceBadge(log.source || "manual").color}`}
                      >
