  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      ...
      try {
        ...
        const singletonTest = await supabase
          .from("patients")
          .select("id")
          .limit(1);
        ...
        if (successfulTable) {
          ...
          setErrorInfo(null);
          setDetails(
            `Conexión establecida con cliente oficial a través de tabla: ${successfulTable}`,
          );
        } else {
        ...
        <div className="error">
          ...
          <div className="mt-4 bg-gray-100 p-3 rounded text-xs">
            ...
            <ul className="list-disc pl-4 mt-1 space-y-1">
              <li>
                Verificar que la API key en el proyecto sea correcta y esté
                activa
              </li>
              <li>
                Comprobar que no hay problemas de CORS en la configuración de
                Supabase
              </li>
              <li>
                Intentar generar una nueva API key en el panel de control de
                Supabase
              </li>
            </ul>
          </div>
          ...
          {details && (
            <details className="mt-4">
              ...
              <pre className="text-xs bg-gray-100 p-2 mt-1 overflow-auto max-h-40">
                {details}
              </pre>
            </details>
          )}
