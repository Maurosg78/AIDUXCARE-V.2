...
describe("AgentSuggestionExplainer", () => {
  ...
  it("no renderiza nada para sugerencias tipo info", () => {
    const infoSuggestion = {
      ...mockSuggestion,
      type: "info" as AgentSuggestion["type"],
    };
    const { container } = render(
      <AgentSuggestionExplainer suggestion={infoSuggestion} />,
    );
  ...
  it("maneja un fallo inicial y reintento exitoso", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("explanation-text")).toHaveTextContent(
        COMPONENT_ERROR_MESSAGES.NETWORK,
      );
    });
    expect(
  ...
  it("maneja varios reintentos fallidos", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("explanation-text")).toHaveTextContent(
        COMPONENT_ERROR_MESSAGES.UNKNOWN,
      );
    });
    expect(
      screen.getByRole("button", { name: "Reintentar" }),
    ...
    vi.mocked(explainSuggestion).mockImplementation(
      () =>
        new Promise<string>((resolve) =>
          setTimeout(() => resolve(mockExplanation), 50),
        ),
    );
  ...
  it("botón se deshabilita durante la carga y se habilita después", async () => {
    ...
    expect(screen.getByTestId("explanation-text")).toHaveTextContent(
      "Explicación cargada manualmente",
    );
    expect(screen.queryByTestId("loading-indicator")).not.toBeInTheDocument();
  });
});
