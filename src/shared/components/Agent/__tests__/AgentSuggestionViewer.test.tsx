...
describe.skip("AgentSuggestionViewer", () => {
  ...
  it("debe manejar la integración de sugerencias correctamente", async () => {
    ...
    const integrateButtons = screen.getAllByRole("button", {
      name: /integrar/i,
    });
    ...
    await waitFor(() => {
      ...
      expect(AuditLogger.log).toHaveBeenCalledWith("suggestion_integrated", {
        suggestionId: mockProps.suggestions[0].id,
  ...
  it("debe manejar el rechazo de sugerencias correctamente", async () => {
    ...
    await waitFor(() => {
      ...
      expect(AuditLogger.log).toHaveBeenCalledWith("suggestion_rejected", {
        suggestionId: mockProps.suggestions[0].id,
        visitId: mockProps.visitId,
        userId: mockProps.userId,
      });
  ...
  it("debe manejar errores de integración correctamente", async () => {
    const mockInsertSuggestion = jest
      .fn()
      .mockRejectedValue(new Error("Error de integración"));
    (EMRFormService.insertSuggestion as jest.Mock).mockImplementation(
      mockInsertSuggestion,
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Error al integrar la sugerencia"),
      ).toBeInTheDocument();
    });
  });
