...
import { SuggestionType, SuggestionField } from "@/types/agent";

// Mock de los servicios externos
const mockInsertSuggestion = vi
  .fn()
  .mockImplementation(() => Promise.resolve(true));
const mockMapSuggestionTypeToEMRSection = vi
  .fn()
  .mockImplementation((type: SuggestionType) => {
...
vi.mock("@/core/services/EMRFormService", () => ({
  EMRFormService: {
    ...
    mapSuggestionTypeToEMRSection: (...args: any[]) =>
      mockMapSuggestionTypeToEMRSection(...args),
  },
}));
...
describe("AgentSuggestionsViewer", () => {
  ...
  it("debe validar la integración completa de sugerencias al EMR", async () => {
    ...
    const toggleButton = screen.getByRole("button", {
      name: "Mostrar sugerencias del copiloto",
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId(`suggestion-${defaultProps.suggestions[0].id}`),
      ).toBeInTheDocument();
    });

    // Encontrar y hacer clic en el botón de integrar
    const integrateButton = screen.getByTestId(
      `accept-suggestion-${defaultProps.suggestions[0].id}`,
    );
    ...
    expect(mockAuditLog).toHaveBeenCalledWith("suggestion_integrated", {
  ...
  it("debe manejar correctamente errores de red al integrar sugerencias", async () => {
    ...
    mockInsertSuggestion.mockRejectedValueOnce(
      new Error("Error de conexión al integrar la sugerencia"),
    );
  ...
  it("debe ser accesible", () => {
    ...
    const suggestionsContent = screen.getByRole("region", {
      name: /Lista de sugerencias/i,
    });
    ...
    const individualSuggestions = suggestionRegions.filter((region) =>
      region.getAttribute("aria-label")?.startsWith("Sugerencia suggestion-"),
    );
    expect(individualSuggestions).toHaveLength(defaultProps.suggestions.length);

    // Verificar que cada sugerencia tiene los atributos ARIA correctos
    individualSuggestions.forEach((region, index) => {
      expect(region).toHaveAttribute(
        "aria-label",
        `Sugerencia ${defaultProps.suggestions[index].id}`,
      );
    });
  });
});
