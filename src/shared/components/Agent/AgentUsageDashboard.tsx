 ...
 */
const AgentUsageDashboard: React.FC<AgentUsageDashboardProps> = ({
  visitId,
}) => {
  ...
  const hasMetrics =
    metrics.generated > 0 || metrics.accepted > 0 || metrics.integrated > 0;
  if (!hasMetrics) {
    return null;
  }
        ...
        <div className="p-3 border-t border-gray-200">
          <div className="space-y-4">
            ...
            <div className="text-xs text-gray-600 mt-2">
              <p>
                De {metrics.generated} sugerencias generadas por el agente,{" "}
                {metrics.accepted} fueron aceptadas ({acceptRate}%) y{" "}
                {metrics.integrated} se integraron en el EMR ({integrationRate}%
                de las aceptadas).
              </p>
            </div>
          </div>
