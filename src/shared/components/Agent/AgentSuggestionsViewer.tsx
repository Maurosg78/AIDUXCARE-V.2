 ...
 * @constant
 */
const INTEGRABLE_SUGGESTION_TYPES = [
  "recommendation",
  "warning",
  "info",
] as const;
        ...
        <button
          ...
          aria-label={
            isIntegrated
              ...
              : !isIntegrable
                ? BUTTON_TEXTS.NOT_INTEGRABLE
                : BUTTON_TEXTS.INTEGRATE
          }
        >
          {isIntegrated
            ? BUTTON_TEXTS.INTEGRATED
            : !isIntegrable
              ? BUTTON_TEXTS.NOT_INTEGRABLE
              : BUTTON_TEXTS.INTEGRATE}
        </button>
...
const AgentSuggestionsViewer: React.FC<AgentSuggestionsViewerProps> = ({
  ...
  const [integratedSuggestions, setIntegratedSuggestions] = useState<
    Set<string>
  >(new Set());
  ...
  const handleSuggestionAccepted = useCallback(
    async (suggestion: AgentSuggestion) => {
      try {
        ...
        if (
          !INTEGRABLE_SUGGESTION_TYPES.includes(
            suggestion.type as IntegrableSuggestionType,
          )
        ) {
          console.warn(
        ...
        const _suggestionToIntegrate: SuggestionToIntegrate = {
          id: suggestion.id,
        ...
        trackMetric(
          "suggestions_integrated",
          {
            ...
            suggestionType: suggestion.type as
              | "recommendation"
              | "warning"
              | "info",
            suggestionField: suggestion.field || "notes",
        ...
        AuditLogger.log("suggestion_integrated", {
          ...
          suggestionField: suggestion.field || "notes",
          content: suggestion.content,
          section: EMRFormService.mapSuggestionTypeToEMRSection(
            suggestion.type,
          ),
        });
      } catch (err) {
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent) => {
    ...
    const buttons = Array.from(
      container.querySelectorAll(
        'button[data-testid^="accept-suggestion-"], button[data-testid^="reject-suggestion-"], button[data-testid="toggle-suggestions"]',
      ),
  ...
  const isSuggestionIntegrable = useCallback((suggestion: AgentSuggestion) => {
    return INTEGRABLE_SUGGESTION_TYPES.includes(
      suggestion.type as IntegrableSuggestionType,
    );
  }, []);
                            ...
                            >
                              <button
                                ...
                                disabled={
                                  ...
                                  !isSuggestionIntegrable(suggestion)
                                }
                                className="inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50"
                                data-testid={`accept-suggestion-${suggestion.id}`}
                                aria-label={
