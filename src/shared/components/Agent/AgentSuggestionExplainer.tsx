...
function detectErrorType(error: unknown): ErrorMessage {
  if (error instanceof Error) {
    ...
    if (msg.includes('network') || msg.includes('connection') || msg.includes('conexión')) {
      return ERROR_MESSAGES.NETWORK;
    }
    if (msg.includes('timeout') || msg.includes('expired') || msg.includes('expirado')) {
...
const AgentSuggestionExplainer: React.FC<Props> = ({ suggestion, className = '' }) => {
  ...
  }, [state.isExpanded, state.isLoading, state.hasFetched, state.error, fetchExplanation]);
