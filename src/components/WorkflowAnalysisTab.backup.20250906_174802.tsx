...
//   
  ...
  const adaptedResults = niagaraResults ? {
    ...niagaraResults,
    physicalTests: niagaraResults.evaluaciones_fisicas_sugeridas || [],    entities: [
      ...(niagaraResults.hallazgos_relevantes || []).filter(h => !h.includes("Medicación actual") || h.toLowerCase().includes("ibuprofeno") || h.toLowerCase().includes("enantyum")).map((h, i) => ({
        id: `symptom-${i}`,
        text: h,
        type: "symptom"
      })),
