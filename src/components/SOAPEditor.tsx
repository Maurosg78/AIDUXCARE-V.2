...
export const SOAPEditor: React.FC<SOAPEditorProps> = ({
  ...
  const exportCopy = {
    ...
    generatedBy: esPilotEnabled ? 'Generado por AiDuxCare' : 'Generated by AiDuxCare',
    documentId: esPilotEnabled ? 'ID del documento' : 'Document ID',
    status: esPilotEnabled ? 'Estado' : 'Status',
    statusFinalized: esPilotEnabled ? 'Finalizada' : 'Finalized',
    ...
    pdfPopup: esPilotEnabled ? 'Permite las ventanas emergentes para exportar el PDF.' : 'Please allow pop-ups to export PDF',
  ...
  const handleDownloadAsText = () => {
    ...
    link.href = url;
    link.download = `${exportCopy.fileName}_${new Date().toISOString().split('T')[0]}.txt`;
  ...
  const handleExportPDF = () => {
    ...
    if (!printWindow) {
      alert(exportCopy.pdfPopup);
      return;
    }
    const currentDate = new Date();
    const pdfDate = currentDate.toLocaleDateString(exportLocale, { year: 'numeric', month: 'long', day: 'numeric' });
    const pdfTime = currentDate.toLocaleTimeString(exportLocale, { hour: '2-digit', minute: '2-digit' });
    const pdfVisitType = visitType === 'initial'
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>${exportCopy.title} - ${currentDate.toLocaleDateString(exportLocale)}</title>
          <style>
            @media print {
              @page { margin: 1in; }
            }
        ...
        <body>
          <h1>${exportCopy.title}</h1>
          <div class="metadata">
            <strong>${exportCopy.date}:</strong> ${pdfDate}<br>
            <strong>${exportCopy.time}:</strong> ${pdfTime}<br>
            <strong>${exportCopy.visitType}:</strong> ${pdfVisitType}
          </div>
          
          <div class="section">
            <div class="section-title">S: ${exportCopy.subjective}</div>
            <div class="section-content">${(currentSOAP.subjective || exportCopy.noSubjective).replace(/\n/g, '<br>')}</div>
          </div>
          
          <div class="section">
            <div class="section-title">O: ${exportCopy.objective}</div>
            <div class="section-content">${(currentSOAP.objective || exportCopy.noObjective).replace(/\n/g, '<br>')}</div>
          </div>
          
          <div class="section">
            <div class="section-title">A: ${exportCopy.assessment}</div>
            <div class="section-content">${(currentSOAP.assessment || exportCopy.noAssessment).replace(/\n/g, '<br>')}</div>
          </div>
          
          <div class="section">
            <div class="section-title">P: ${exportCopy.plan}</div>
            <div class="section-content">${(currentSOAP.plan || exportCopy.noPlan).replace(/\n/g, '<br>')}</div>
          </div>
          
          ${currentSOAP.referrals ? `
          <div class="section">
            <div class="section-title">${exportCopy.referrals}</div>
          ...
          <div class="section">
            <div class="section-title">${exportCopy.precautions}</div>
          ...
          <div class="section">
            <div class="section-title">${exportCopy.additionalNotes}</div>
            <div class="section-content">${currentSOAP.additionalNotes.replace(/\n/g, '<br>')}</div>
          </div>
          ` : ''}
          
          <div class="metadata">
            <em>${exportCopy.generatedBy} - ${exportCopy.pdfSubtitle}</em>
          </div>
