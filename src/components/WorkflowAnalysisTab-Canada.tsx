...
export default WorkflowAnalysisTab;
  ...
  // Hook canadiense adicional para funcionalidad canadiense
  const { analyzeWithProfile: analyzeCanadian, isProfileReady, specialtyInfo } = useCanadianVertexAI();
  
  // Log de testing para verificar que el hook canadiense está activo
  useEffect(() => {
    if (isProfileReady) {
      console.log('🇨🇦 Canadian Profile Active:', specialtyInfo);
    }
  }, [isProfileReady, specialtyInfo]);
