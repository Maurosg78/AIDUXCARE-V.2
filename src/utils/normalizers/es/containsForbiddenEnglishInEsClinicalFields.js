...
const collectEsClinicalFields = (analysis) => {
    ...
    const physicalTestTexts = physicalTests.flatMap((item) => {
...
export const containsForbiddenEnglishInEsClinicalFields = (analysis) => {
...
export const getForbiddenEnglishTermsInEsClinicalFields = (analysis) => {
    const clinicalFields = collectEsClinicalFields(analysis);
    const matches = [];
    for (const fieldValue of clinicalFields) {
        for (const forbiddenTerm of FORBIDDEN_ENGLISH_TERMS) {
