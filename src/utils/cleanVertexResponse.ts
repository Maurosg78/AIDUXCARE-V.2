...
export function cleanVertexResponse(input: any): NiagaraResult {
  ...
  const ensure = (a:any)=>Array.isArray(a)?a.map(x=>typeof x==='string'?x.trim():x).filter(Boolean):[];
  out.hallazgos_clinicos = ensure(out.hallazgos_clinicos);
  out.hallazgos_relevantes = ensure(out.hallazgos_relevantes);
  out.contexto_ocupacional = ensure(out.contexto_ocupacional);
  out.contexto_psicosocial = ensure(out.contexto_psicosocial);
  out.medicacion_actual = ensure(out.medicacion_actual);
  out.antecedentes_medicos = ensure(out.antecedentes_medicos);
  out.diagnosticos_probables = ensure(out.diagnosticos_probables);
  out.red_flags = ensure(out.red_flags);
  out.yellow_flags = ensure(out.yellow_flags);
  out.plan_tratamiento_sugerido = ensure(out.plan_tratamiento_sugerido);
