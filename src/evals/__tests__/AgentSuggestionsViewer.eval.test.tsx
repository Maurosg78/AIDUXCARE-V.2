...
vi.mock("../../core/services/EMRFormService", () => ({
  EMRFormService: {
    ...
    mapSuggestionTypeToEMRSection: vi
      .fn()
...
describe("AgentSuggestionsViewer - Evaluación", () => {
  ...
  it("debe mostrar los textos esperados según el tipo de sugerencia", () => {
    ...
    expect(
      within(recommendationSection).getByText(/radiografía de tórax/i),
    ).toBeInTheDocument();
    expect(
      within(warningSection).getByText(/alergias a medicamentos/i),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText((content) => content.includes("radiografía de tórax")),
    ).toBeInTheDocument();
    expect(
      screen.getByText((content) =>
        content.includes("alergias a medicamentos"),
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText((content) => content.includes("Última visita")),
    ).toBeInTheDocument();
    expect(
      screen.getByText((content) =>
        content.includes("seguimiento de presión arterial"),
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText((content) => content.includes("HbA1c elevada")),
    ).toBeInTheDocument();
  });
  ...
  it("debe manejar correctamente la integración de sugerencias", async () => {
    ...
    const acceptButton = getByTestId(
      `accept-suggestion-${mockSuggestions[0].id}`,
    );
  ...
  it("debe manejar correctamente el rechazo de sugerencias", async () => {
    ...
    const rejectButton = getByTestId(
      `reject-suggestion-${mockSuggestions[0].id}`,
    );
  ...
  it("debe mostrar las sugerencias agrupadas por tipo", async () => {
    ...
    expect(
      within(recommendationSection).getByText(mockSuggestions[0].content),
    ).toBeInTheDocument();
    expect(
      within(warningSection).getByText(mockSuggestions[1].content),
    ).toBeInTheDocument();
    expect(
      within(infoSection).getByText(mockSuggestions[2].content),
    ).toBeInTheDocument();
  });
  ...
  it("debe mostrar un mensaje cuando no hay sugerencias", () => {
    ...
    expect(
      screen.getByText("Sugerencias del Copiloto (0)"),
    ).toBeInTheDocument();
