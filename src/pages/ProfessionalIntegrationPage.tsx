...
export const ProfessionalIntegrationPage: React.FC<
  ProfessionalIntegrationPageProps
> = ({
  ...
  const initializeSystems = useCallback(async () => {
    ...
    try {
      ...
      setError(
        `Error de inicialización: ${error instanceof Error ? error.message : "Error desconocido"}`,
      );
  ...
  const handleSuggestionsGenerated = useCallback(
    (newSuggestions: AgentSuggestion[]) => {
        ...
        "suggestions_accepted",
        {
          ...
          suggestionType:
            suggestion.type === "diagnostic"
              ? "recommendation"
              : (suggestion.type as "recommendation" | "warning" | "info"),
          suggestionField: suggestion.field,
        },
        userId,
        visitId,
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className={`text-xs ${getStatusColor(systemStatus.ollama)}`}>
                {systemStatus.ollama === "checking"
                  ? "Verificando..."
                  : systemStatus.ollama === "healthy"
                    ? "Operativo"
                    : "Error"}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className={`text-xs ${getStatusColor(systemStatus.nlp)}`}>
                {systemStatus.nlp === "checking"
                  ? "Verificando..."
                  : systemStatus.nlp === "healthy"
                    ? "Operativo"
                    : "Error"}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className={`text-xs ${getStatusColor(systemStatus.audio)}`}>
                {systemStatus.audio === "checking"
                  ? "Verificando..."
                  : systemStatus.audio === "healthy"
                    ? "Disponible"
                    : "No disponible"}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div
                className={`text-xs ${getStatusColor(systemStatus.integration)}`}
              >
                {systemStatus.integration === "checking"
                  ? "Verificando..."
                  : systemStatus.integration === "healthy"
                    ? "Completa"
                    : "Incompleta"}
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-12 pt-8 border-t border-gray-200">
          <div className="text-center text-sm text-gray-500">
            <div className="mb-2">
              🤖 <strong>Powered by Ollama Local LLM</strong> - Costo: $0.00 por
              transcripción
            </div>
            <div>
              Escalable para múltiples fisioterapeutas • Integración profesional
              completa
            </div>
          </div>
        </div>
