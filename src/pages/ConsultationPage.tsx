...
const ConsultationPage: React.FC = () => {
  ...
  const [highlights] = useState([
    { id: '1', text: 'Dolor lumbar irradiado', category: 'síntoma', confidence: 0.95 },
    { id: '2', text: 'Limitación flexión', category: 'hallazgo', confidence: 0.88 },
    { id: '3', text: 'Test Lasègue positivo', category: 'hallazgo', confidence: 0.92 }
  ]);
  const [legalWarnings] = useState([
    { id: '1', description: 'Paciente alérgico a AINEs - evitar antiinflamatorios', severity: 'alta' },
    { id: '2', description: 'Diabetes - monitorear ejercicio intenso', severity: 'media' }
  ]);
      ...
      content: (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
          ...
          <div className="bg-white rounded-lg border p-6">
            ...
            <button className="w-full py-2 px-4 rounded bg-green-500 text-white font-medium">Iniciar Escucha</button>
            <p className="text-xs text-gray-500 mt-2">Activa la transcripción automática del ambiente.</p>
          </div>
            ...
            <ul className="space-y-2">
              {highlights.map(h => (
                <li key={h.id} className="text-sm flex items-center">
                  ...
                  {h.text} <span className="ml-2 text-xs text-gray-400">({h.category})</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="bg-white rounded-lg border p-6">
            <h3 className="font-semibold mb-2 text-red-600">Advertencias Clínicas</h3>
            <ul className="space-y-2">
              {legalWarnings.map(w => (
                <li key={w.id} className="text-sm flex items-center">
                  <span className="w-2 h-2 rounded-full bg-red-400 mr-2"></span>
                  {w.description} <span className="ml-2 text-xs text-gray-400">({w.severity})</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      content: (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          ...
          <div className="bg-white rounded-lg border p-6 flex flex-col min-h-[300px]">
            ...
            <textarea className="w-full h-40 border rounded p-2 mb-2" placeholder="Escribe o edita la nota SOAP aquí..." />
            <button className="mt-2 px-4 py-2 rounded bg-primary text-white font-medium self-end">Guardar Nota</button>
          </div>
        </div>
      )
    }
      ...
      <div className="bg-white border-b p-4 flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{patientData.age} años • {patientData.condition}</p>
          </div>
        </div>
        <span className="px-3 py-1 rounded-md text-sm font-medium bg-primary text-white">{patientData.id}</span>
      </div>
        ...
        {showAssistant && (
          <div className="absolute bottom-16 right-0 w-80 bg-white rounded-lg shadow-xl p-4">
            ...
            <div className="text-sm text-gray-700">
              ...
              <input className="mt-4 w-full border rounded p-2" placeholder="Escribe tu consulta..." />
              <button className="mt-2 px-4 py-2 rounded bg-primary text-white font-medium w-full">Enviar</button>
            </div>
          </div>
        )}
      </div>
