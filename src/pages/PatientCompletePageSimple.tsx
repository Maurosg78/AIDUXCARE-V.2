/**
 * 🏥 PATIENT COMPLETE PAGE - Solo Audio Real
 * Página de consulta completa con funcionalidad de audio real únicamente
 */
...
const PatientCompletePageSimple: React.FC = () => {
    ...
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onerror = (event: any) => {
          ...
          switch (event.error) {
            case 'network':
              errorMessage = 'Problema de conexión. Reintentando automáticamente...';
              canRetry = true;
              break;
            case 'not-allowed':
            ...
            case 'no-speech':
              ...
              break;
            case 'audio-capture':
              errorMessage = 'Error del micrófono. Verifica que esté conectado.';
              break;
            case 'service-not-allowed':
              errorMessage = 'Servicio no disponible. Intenta en modo incógnito.';
              break;
            default:
              errorMessage = `Error técnico: ${event.error}. Recarga la página.`;
          }
          ...
          if (canRetry && audioState.isRecording) {
            setTimeout(() => {
              console.log('RELOAD: Reintentando reconocimiento...');
              setAudioState(prev => ({
                ...prev,
                error: 'Reconectando...'
              }));
              ...
              setTimeout(() => {
                if (recognitionRef.current && audioState.isRecording) {
                  try {
  ...
  const toggleRecording = async () => {
    if (!recognitionRef.current) {
      setAudioState(prev => ({
        ...prev,
        error: 'Reconocimiento de voz no disponible. Usa Chrome o Edge.'
      }));
      return;
    }
    ...
    if (audioState.isRecording) {
      ...
      try {
        ...
        setAudioState(prev => ({
          ...prev,
          error: 'Micrófono no autorizado. Permite el acceso al micrófono en tu navegador.'
        }));
        return;
      }
      ...
      try {
        ...
        setAudioState(prev => ({
          ...prev,
          error: 'Error al iniciar grabación. Recarga la página e intenta de nuevo.',
          isRecording: false
        }));
      }
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            {/* Control de Audio - SOLO AUDIO REAL */}
            <div className="bg-white rounded-lg shadow-sm border border-[#BDC3C7]/20 p-6">
              <div className="text-center">
                ...
                {audioState.recordingTime > 0 && (
                  <div className="mb-4 text-2xl font-mono text-[#5DA5A3]">
                ...
                {audioState.isProcessing && (
                  <div className="flex items-center justify-center space-x-2 text-[#5DA5A3]">
                    ...
                    <span className="text-sm">Analizando transcripción y generando SOAP...</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {audioState.transcript && (
              <div className="bg-white rounded-lg shadow-sm border border-[#BDC3C7]/20 p-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4 max-h-60 overflow-y-auto">
                  <p className="text-sm text-[#2C3E50] whitespace-pre-wrap">{audioState.transcript}</p>
                </div>
              </div>
            )}
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 border-t border-gray-200">
                <button
                  ...
                  className="w-full px-4 py-2 bg-[#5DA5A3] text-white rounded-lg hover:bg-[#4A8280] transition-colors font-medium"
                >
                  💾 Guardar Consulta
                </button>
              </div>
...
export default PatientCompletePageSimple; 