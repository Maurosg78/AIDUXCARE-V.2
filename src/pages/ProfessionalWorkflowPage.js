...
import { Loader2, CheckCircle, AlertCircle, FileText, LogOut, ArrowLeft } from "lucide-react";
...
import ClinicalAttachmentService from "../services/clinicalAttachmentService";
import { FileProcessorService } from "../services/FileProcessorService";
import { matchTestName } from "@/core/msk-tests/matching/fuzzyMatch";
import { detectVisitType } from "../core/soap/SOAPContextBuilder";
...
import { organizeSOAPData, validateUnifiedData, createDataSummary } from "../core/soap/SOAPDataOrganizer";
...
import UniversalShareMenu from "../components/share/UniversalShareMenu";
...
import { MSK_TEST_LIBRARY, regionLabels, hasFieldDefinitions, getTestDefinition, } from "@/core/msk-tests/library/mskTestLibrary";
...
import { routeWorkflow, shouldSkipTab, getInitialTab } from "../services/workflowRouterService";
import WorkflowFeedback from "../components/workflow/WorkflowFeedback";
import { trackWorkflowSessionStart, trackSOAPGeneration, trackWorkflowSessionEnd } from "../services/workflowMetricsService";
import { trackSessionStarted, trackSOAPGenerationStarted, trackSOAPGenerationCompleted, trackRecordingStarted, trackRecordingStopped, trackTranscriptionStarted, trackTranscriptionCompleted, trackTranscriptionFailed, trackAnalysisRequested, trackAnalysisCompleted, trackAnalysisFailed, trackError, } from "@/services/analytics/AnalyticsEvents";
...
const WORKFLOW_STORAGE_KEY = (patientId) => `aidux_workflow_${patientId}`;
const demoPatient = {
    id: "CA-TEST-001",
    name: "Sofia Bennett",
...
const RESULT_OPTIONS = ["normal", "positive", "negative", "inconclusive"];
const isValidResult = (value) => value === "normal" || value === "positive" || value === "negative" || value === "inconclusive";
const sanitizeSource = (value) => value === "ai" || value === "custom" ? value : "manual";
const sanitizeEvaluationEntry = (entry) => ({
    id: entry.id,
    name: entry.name,
...
const formatDetectedLanguage = (value) => {
    if (!value)
        return "Not detected";
    ...
    if (normalized.startsWith("fr"))
        return "Detected: French";
    if (normalized.startsWith("es"))
        return "Detected: Spanish";
...
const ProfessionalWorkflowPage = () => {
    ...
    const hasCleanedForInitial = useRef(null);
    // Guard against duplicate SOAP saves (double-tap / double-click)
    const isFinalizingRef = useRef(false);
    // WO-IA-RESUME-01: Only run resume load once per sessionId
    const hasResumeLoadAttemptedRef = useRef(null);
    const restoreTranscriptPollRef = useRef(null);
    ...
    const [workflowConsentStatus, setWorkflowConsentStatus] = useState(null);
    ...
    const [activeTab, setActiveTab] = useState(isExplicitFollowUp ? "analysis" : "analysis");
    const [selectedEntityIds, setSelectedEntityIds] = useState([]);
    const [selectedRedFlagIds, setSelectedRedFlagIds] = useState([]);
    const [redFlagDecisions, setRedFlagDecisions] = useState({});
    const [followUpDecisionResolved, setFollowUpDecisionResolved] = useState(false);
    const [localSoapNote, setLocalSoapNote] = useState(null);
    const [soapStatus, setSoapStatus] = useState('draft');
    const [visitType, setVisitType] = useState(isExplicitFollowUp ? 'follow-up' : 'initial');
    const hasUndecidedFollowUpRedFlags = (forceFollowUp) => {
        const isFollowUp = forceFollowUp || visitType === 'follow-up';
        if (!isFollowUp)
            return false;
        if (!interactiveResults?.redFlags?.length)
            return false;
        return interactiveResults.redFlags.some((flag) => {
    ...
    useEffect(() => {
        ...
        const allResolved = followUpAlerts.red_flags.every((flag, idx) => {
    ...
    const [inClinicItems, setInClinicItems] = useState([]);
    const [homeProgramItems, setHomeProgramItems] = useState([]);
    // Follow-up path: baseline for SOAP (no Niagara). Single source of truth — loaded when visitType === 'follow-up'.
    const [followUpClinicalState, setFollowUpClinicalState] = useState(null);
    ...
    const [workflowRoute, setWorkflowRoute] = useState(null);
    ...
    const [resumeLoadFailed, setResumeLoadFailed] = useState(null);
    const setAnalysisErrorWithRecovery = useCallback((err) => {
    ...
    const [currentSessionForComparison, setCurrentSessionForComparison] = useState(null);
    // WO-IA-CLOSE-01: Initial assessment closed — button hide and session state
    const [initialAssessmentClosedAt, setInitialAssessmentClosedAt] = useState(null);
    const [baselineIdFromSession, setBaselineIdFromSession] = useState(null);
    const [showCloseInitialConfirmModal, setShowCloseInitialConfirmModal] = useState(false);
    const [closeInitialConfirmData, setCloseInitialConfirmData] = useState(null);
    ...
    const [transcriptionEndTime, setTranscriptionEndTime] = useState(null);
    const [soapGenerationStartTime, setSoapGenerationStartTime] = useState(null);
    const [soapGenerationEndTime, setSoapGenerationEndTime] = useState(null);
    ...
    const [isFirstSession, setIsFirstSession] = useState(null); // null = checking, true/false = result
    const [checkingFirstSession, setCheckingFirstSession] = useState(true);
    const [patientHasConsent, setPatientHasConsent] = useState(null); // null = checking
    const [consentStatus, setConsentStatus] = useState(null); // Current consent status
    ...
    const [copyConsentFeedback, setCopyConsentFeedback] = useState('idle');
    ...
    const sessionIdForTranscriptRef = useRef(null);
    const patientIdForPersistRef = useRef(null);
    const userForPersistRef = useRef(null);
    ...
    const { transcript, isRecording, isTranscribing, error: transcriptError, languagePreference, setLanguagePreference, mode, setMode, meta: transcriptMeta, audioStream, startRecording: _startRecording, stopRecording: _stopRecording, setTranscript, } = useTranscript({
    ...
    const { processText, generateSOAPNote, niagaraResults, soapNote, isProcessing, reset: resetNiagaraProcessor, } = useNiagaraProcessor();
    const clinicName = useMemo(() => deriveClinicName(professionalProfile), [professionalProfile]);
    // Prioridad 1.1: Hardening extra - validar que el profile pertenece al user.uid
    const safeProfile = useMemo(() => (professionalProfile?.uid === user?.uid ? professionalProfile : null), [professionalProfile, user?.uid]);
    const clinicianDisplayName = useMemo(() => deriveClinicianDisplayName(safeProfile, user), [safeProfile, user]);
    ...
    const consentLink = useMemo(() => (consentToken ? `${consentBaseUrl}/consent/${consentToken}` : null), [consentBaseUrl, consentToken]);
    ...
    const handleResendConsentSMS = useCallback(async () => {
        ...
        try {
            ...
            const token = await PatientConsentService.generateConsentToken(patientId, currentPatient.fullName || `${currentPatient.firstName} ${currentPatient.lastName}`.trim(), formattedPhone, currentPatient.email || undefined, clinicName, user.uid, clinicianDisplayName);
            ...
            await SMSService.sendConsentLink(formattedPhone, currentPatient.fullName || `${currentPatient.firstName} ${currentPatient.lastName}`.trim(), clinicName, physioNameForSms, token);
            setConsentPending(true);
            setSmsError(null);
    ...
    const trackedSessionsRef = useRef(new Set());
    // ✅ STRICTMODE FIX: Idempotent tracking to prevent duplicate events in dev
    const trackedOnceRef = useRef(new Set());
    const trackOnce = useCallback((key, fn) => {
        if (trackedOnceRef.current.has(key))
            return;
    ...
    useEffect(() => {
        ...
        if (!sessionTrackingKey || !patientId || !user?.uid)
            return;
        ...
        const detectWorkflow = async () => {
            ...
            try {
                ...
                trackOnce(`workflow_session_started:${patientId}:${workflowTypeForTracking}:${route.type}`, () => {
                    trackWorkflowSessionStart(sessionIdForMetrics, patientId, user.uid, workflowTypeForTracking, route.skipTabs.length).catch((error) => {
    ...
    useEffect(() => {
        ...
        setFollowUpClinicalState(null);
        setFollowUpBaselineChecked(false);
        const baselineFromOngoing = location.state?.baselineFromOngoing;
    ...
    const handleWorkflowSelected = useCallback((route) => {
        setWorkflowRoute(route);
        // Update visit type
    ...
    const [evaluationTests, setEvaluationTests] = useState(() => (sharedState.physicalEvaluation?.selectedTests ?? []).map(sanitizeEvaluationEntry));
    ...
    const unmountPersistRef = useRef(null);
    useEffect(() => {
        ...
        return () => {
            ...
            if (isInitialSession) {
                ...
                if (hasValidSessionId) {
                    ...
                    const hasCatchHandler = updateSessionPromise != null &&
    ...
    const [customTestRegion, setCustomTestRegion] = useState("shoulder");
    ...
    const detectedCaseRegion = useMemo(() => {
    ...
    const buildCurrentSession = useCallback(() => {
        if (!currentPatient?.id && !patientIdFromUrl)
            return null;
        if (!localSoapNote)
            return null; // Only show comparison when SOAP is generated
    ...
    useEffect(() => {
        ...
        const restoreFromFirestore = async () => {
            try {
                ...
                if (cancelled)
                    return;
                const match = sessions.find((s) => s.patientId === patientIdFromUrl && s.sessionType === 'followup');
    ...
    useEffect(() => {
        if (!resumeFromUrl || visitType !== 'initial')
            return;
        if (!sessionIdFromUrl) {
        ...
        if (hasResumeLoadAttemptedRef.current === sessionIdFromUrl)
            return;
        ...
        (async () => {
            try {
                ...
                if (sessionData && sessionData.soapNote) {
                    ...
                    setLocalSoapNote(sessionData.soapNote);
                    setSoapStatus((sessionData.status === 'completed' ? 'finalized' : 'draft'));
                    if (hasUndecidedFollowUpRedFlags()) {
                        console.warn('[RED-FLAG-GATE] Follow-up blocked — decisions pending');
                        return;
                    }
                    ...
                    if (sessionData.physicalTests && Array.isArray(sessionData.physicalTests) && sessionData.physicalTests.length > 0) {
                        const sanitized = sessionData.physicalTests.map(sanitizeEvaluationEntry);
                        setEvaluationTests(sanitized);
                        updatePhysicalEvaluation(sanitized);
                    }
                    return;
    ...
    useEffect(() => {
        if (!isRecording || sessionId || !user?.uid || !patientIdFromUrl)
            return;
        ...
        const patientName = currentPatient?.fullName || `${currentPatient?.firstName ?? ''} ${currentPatient?.lastName ?? ''}`.trim() || 'Patient';
        sessionService
    ...
    const lastSharedStateRef = useRef(''); // Track last sharedState to prevent unnecessary updates
    ...
    const consentPollingAttemptsRef = useRef(0);
    const consentPollingPatientIdRef = useRef(null);
    ...
    const hasValidConsent = consentGrantedRef.current === true ||
    ...
    const persistEvaluation = useCallback((next) => {
        // ✅ PHASE 2: Enhanced logging for debugging
        ...
        setEvaluationTests((currentTests) => {
            ...
            const hasValueChanges = sanitized.some(newTest => {
                ...
                if (!currentTest)
                    return false; // New test, already detected above
     ...
     * WO-FIX-TESTS-DUPLICADOS-03
     */
    const testAlreadyExists = (testName, existingTests) => {
    ...
    const addEvaluationTest = useCallback((entry) => {
        // ✅ PHASE 2: Enhanced logging for debugging
        ...
        setEvaluationTests((currentTests) => {
            const exists = currentTests.some((test) => test.id === entry.id || normalizeName(test.name) === normalizeName(entry.name));
            if (exists) {
    ...
    const removeEvaluationTest = useCallback((id) => {
        setEvaluationTests((currentTests) => {
    ...
    const updateEvaluationTest = useCallback((id, updates) => {
        setEvaluationTests((currentTests) => {
            const next = currentTests.map((test) => test.id === id ? { ...test, ...updates } : test);
            persistEvaluation(next);
            return next;
        });
    }, [persistEvaluation]);
    const createEntryFromLibrary = useCallback((test, source) => {
        ...
        const initialValues = {};
        const prefillDefaults = {}; // Track pre-filled defaults
    ...
    const createCustomEntry = (name, source, region = null) => ({
        id: source === "custom"
    ...
    const isTestAlreadySelected = useCallback((id, name) => evaluationTests.some((test) => test.id === id || normalizeName(test.name) === normalizeName(name)), [evaluationTests]);
    ...
    const resolveVoiceLanguage = useCallback(() => {
    ...
    const aiSuggestions = useMemo(() => {
        ...
        if (!niagaraResults?.evaluaciones_fisicas_sugeridas)
            return [];
        ...
        return niagaraResults.evaluaciones_fisicas_sugeridas
            ...
            if (!test)
                return null; // Mark null tests for filtering
            ...
            .filter((item) => item !== null); // Filter nulls but keep original indices
    ...
    const interactiveResults = useMemo(() => {
        ...
        const allPhysicalTests = rawTests
            ...
            if (typeof test === "string") {
                return {
                    ...
                    sensitivityQualitative: libraryMatch?.sensitivityQualitative || undefined,
                    specificityQualitative: libraryMatch?.specificityQualitative || undefined,
                    evidence_level: libraryMatch?.evidence_level || test.evidence_level || test.evidencia,
                    evidencia: test.evidencia || test.evidence_level || libraryMatch?.evidence_level,
                    indication: "",
                    justification: "",
            ...
            return {
                ...
                sensitivity: libraryMatch?.sensitivity !== undefined
                    ...
                    : test.sensibilidad !== undefined
                        ...
                        : test.sensitivity !== undefined
                            ...
                            : undefined,
                specificity: libraryMatch?.specificity !== undefined
                    ...
                    : test.especificidad !== undefined
                        ...
                        : test.specificity !== undefined
                            ...
                            : undefined,
                sensitivityQualitative: libraryMatch?.sensitivityQualitative || test.sensitivityQualitative || undefined,
                specificityQualitative: libraryMatch?.specificityQualitative || test.specificityQualitative || undefined,
                evidence_level: test.evidence_level || test.evidencia || libraryMatch?.evidence_level,
                evidencia: test.evidencia || test.evidence_level || libraryMatch?.evidence_level,
        ...
        console.log('[interactiveResults] Physical tests calculation:', {
            ...
            topTestsNames: finalPhysicalTests.map((t) => ({
                name: t.name || t.test,
            ...
            remainingTestsNames: additionalPhysicalTests.map((t) => ({
                name: t.name || t.test,
                originalIndex: t.originalIndex
            }))
        });
        ...
        const symptomEntities = (niagaraResults.hallazgos_clinicos || []).map((text, index) => ({
            id: `symptom-${index}`,
            text,
            type: "symptom"
        })) || [];
        const medicationEntities = (niagaraResults.medicacion_actual || []).map((text, index) => ({
            id: `medication-${index}`,
            text,
            type: "medication"
        })) || [];
        const historyEntities = (niagaraResults.antecedentes_medicos || []).map((text, index) => ({
            id: `history-${index}`,
            text,
            type: "history"
        })) || [];
    ...
    const physicalExamResults = useMemo(() => filteredEvaluationTests.map((entry) => {
    ...
    const completedCount = useMemo(() => filteredEvaluationTests.filter((entry) => entry.result && entry.result !== "").length, // ✅ P1.1: Use filtered tests
    ...
    const handleAnalyzeWithVertex = async () => {
        ...
        try {
            ...
            if (error?.message?.includes('network') || error?.message?.includes('fetch')) {
                setAnalysisError(message + ' Check your internet connection. You can still document manually.');
            }
        }
    };
    ...
    const continueToEvaluation = () => {
        ...
        const selectedKeys = new Set(selectedEntityIds
    ...
    const [treatmentReminder, setTreatmentReminder] = useState(null);
    const [previousTreatmentPlan, setPreviousTreatmentPlan] = useState(null);
    ...
    const [referralReportData, setReferralReportData] = useState(null);
    ...
    const buildReferralReportData = useCallback((clinicalEvolutionSummary) => {
        ...
        const redFlagsSource = interactiveResults.redFlags;
        const redFlags = redFlagsSource?.flatMap((flag, idx) => {
            const id = typeof flag === "string" ? flag : (flag?.label ?? `red-${idx}`);
            const decision = redFlagDecisions[id]?.decision;
            ...
            const evidence = typeof flag === "object" && flag && "evidence" in flag
                ? flag.evidence
                : undefined;
            const urgency = typeof flag === "object" && flag && "urgency" in flag
                ? flag.urgency
                : undefined;
            return [
                {
        ...
        const chiefComplaint = interactiveResults.chief_complaint ||
            interactiveResults.chiefComplaint ||
            undefined;
    ...
    const handleOpenReferralReport = useCallback(async () => {
        ...
        try {
            const patientId = patientIdFromUrl ?? currentPatient?.id ?? '';
            const evolution = patientId ? await new SessionComparisonService().getReferralEvolutionSentence(patientId) : null;
            ...
            const loadTreatmentPlan = async () => {
                try {
                    ...
                    if (plan) {
                        ...
                        const reminder = await treatmentPlanService.getTreatmentReminder(patientId, visitCount.data ? visitCount.data + 1 : 2);
                        if (reminder) {
    ...
    useEffect(() => {
        ...
        const planText = followUpClinicalState?.baselineSOAP?.plan?.trim() ||
        ...
        setInClinicItems(derived.inClinic.map((label, i) => ({
            id: `in-clinic-${i}`,
            label,
            completed: false,
            source: 'plan',
        })));
        setHomeProgramItems(derived.homeProgram.map((label, i) => ({
            id: `hep-${i}`,
            label,
            completed: false,
            source: 'plan',
    ...
    const handleGenerateSoap = async () => {
        ...
        if (!consentCheck.hasValidConsent) {
            ...
            setAnalysisError(t('workflow.consentRequiredForSoap'));
        ...
        try {
            ...
            if (focusToInject.length > 0) {
                ...
                console.info('[WO-05-FIX][PROOF] Injecting todayFocus into SOAPContext', {
            ...
            await trackSOAPGeneration(sessionIdForMetrics, response.metadata.tokens, response.metadata.tokenOptimization);
            ...
            try {
                ...
                if (!tokensCharged) {
                    ...
                    // Cobrar tokens
                    await tokenTrackingService.recordTokenUsage(userIdForCharging, sessionIdForMetrics, tokensToCharge);
            ...
            const objectiveValidation = validateSOAPObjective(response.soap.objective || '', organized.structuredData.physicalExamResults);
            ...
            if (error?.message?.includes('network') || error?.message?.includes('fetch')) {
                setAnalysisError(errorMessage + ' Check your internet connection. You can still create SOAP notes manually.');
            }
        }
        finally {
            setIsGeneratingSOAP(false);
    ...
    const handleGenerateSOAPFollowUp = useCallback(async () => {
        ...
        try {
            ...
            let longitudinalSummary;
            let trajectoryPattern;
            let trajectoryConfidence;
            let painSeriesSummary;
            try {
                ...
                if (pid) {
                    ...
                    if (!state.isFirstSession && state.previousSession && state.currentSession) {
                        const comparison = comparisonService.compareSessions(state.previousSession, state.currentSession);
                        const uiData = comparisonService.formatComparisonForUI(comparison, false);
            ...
            if (safeAlerts.red_flags.length > 0) {
                setFollowUpAlerts({ ...alerts, red_flags: safeAlerts.red_flags });
    ...
    const calculateAndTrackValueMetrics = useCallback(async (finalizedAt) => {
        try {
            ...
            const transcriptionTime = transcriptionStartTime && transcriptionEndTime
                ...
                : undefined;
            const aiGenerationTime = soapGenerationStartTime && soapGenerationEndTime
                ? (soapGenerationEndTime.getTime() - soapGenerationStartTime.getTime()) / 1000 / 60
                : undefined;
            const totalDocumentationTime = (new Date().getTime() - sessionStartTime.getTime()) / 1000 / 60;
            const rawManualEditingTime = totalDocumentationTime -
                (aiGenerationTime || 0) -
                (transcriptionTime || 0);
            const manualEditingTime = rawManualEditingTime > 0 ? rawManualEditingTime : 0;
            const metrics = {
                transcriptionTime,
                aiGenerationTime,
                manualEditingTime,
    ...
    const handleSaveSOAP = async (soap, status) => {
    ...
    const handleUnfinalizeSOAP = async (soap) => {
    ...
    const handleFinalizeSOAP = async (soap) => {
        ...
        try {
            ...
            try {
                if (!sessionId || !user?.uid || !workflowRoute) {
                    console.warn('[WORKFLOW] Skipping workflow end tracking (missing sessionId or user)', { sessionId, user: user?.uid, workflowRoute });
                }
                else {
                    const metrics = await trackWorkflowSessionEnd(sessionId, user.uid, patientIdFromUrl || demoPatient.id);
                    if (metrics) {
            ...
            try {
                ...
                const followUpRaw = soap.followUp?.trim() ?? '';
                ...
                const result = await saveSOAPNoteWithRetry(soapDataToSave, patientId, sessionId, {
                    maxRetries: 3,
                    retryDelay: 1000,
                    enableBackup: true,
                ...
                setAnalysisError('Note finalized but failed to save to Clinical Vault. ' +
            ...
            if (soap.plan) {
                try {
                    await treatmentPlanService.saveTreatmentPlan(patientIdFromUrl || demoPatient.id, currentPatient?.fullName || `${currentPatient?.firstName || ''} ${currentPatient?.lastName || ''}`.trim() || demoPatient.name, TEMP_USER_ID, soap.plan, visitType);
    ...
    const formatLastSessionDate = useCallback((encounter) => {
    ...
    if (isExplicitFollowUp && !patientIdFromUrl) {
        return _jsx(Navigate, { to: "/command-center", replace: true });
    }
    ...
    if (consentResolution?.hardBlock === true) {
        ...
        if (showVerbalConsentForDeclined) {
            return (_jsx(VerbalConsentModal, { patientId: patientIdFromUrl, patientName: currentPatient?.fullName || `${currentPatient?.firstName || ''} ${currentPatient?.lastName || ''}`.trim(), patientEmail: currentPatient?.email, patientPhone: currentPatient?.phone || currentPatient?.personalInfo?.phone, onClose: () => {
        ...
        return (_jsx(DeclinedConsentModal, { patientName: currentPatient?.fullName || `${currentPatient?.firstName || ''} ${currentPatient?.lastName || ''}`.trim(), declineReasons: workflowConsentStatus?.declineReasons, patientId: patientIdFromUrl, onRecordNewConsent: () => {
    ...
    if (patientIdFromUrl && user?.uid && currentPatient && consentCheckComplete &&
        ...
        return (_jsx(ConsentGateScreen, { patientId: patientIdFromUrl, patientName: currentPatient?.fullName || `${currentPatient?.firstName || ''} ${currentPatient?.lastName || ''}`.trim(), patientPhone: currentPatient?.phone || currentPatient?.personalInfo?.phone, clinicName: clinicName, consentResolution: consentResolution, physiotherapistId: user?.uid, physiotherapistName: clinicianDisplayName, onConsentGranted: handleConsentGrantedImmediate, 
    ...
    if (patientIdFromUrl && user?.uid && currentPatient && !consentCheckComplete) {
        return (_jsx("div", { className: "min-h-screen bg-slate-50 flex items-center justify-center", children: _jsxs("div", { className: "text-center", children: [_jsx(LoadingSpinner, {}), _jsx("p", { className: "mt-4 text-sm text-gray-600", children: "Verifying patient consent..." })] }) }));
    }
    ...
    const effectiveActiveTab = isFollowUpWithRedFlags ? 'analysis' : activeTab;
    return (_jsxs("div", { className: "min-h-screen bg-slate-50", children: [showCPOBlockModal && (_jsx("div", { className: "fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4", children: _jsxs("div", { className: "w-full max-w-md bg-white rounded-2xl shadow-2xl", children: [_jsx("div", { className: "bg-red-600 rounded-t-2xl p-6 text-white", children: _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("div", { className: "bg-white/20 rounded-full p-2", children: _jsx("span", { className: "text-2xl", children: "\u26A0\uFE0F" }) }), _jsxs("div", { children: [_jsx("h2", { className: "text-xl font-bold", children: t('clinical.soap.cannotFinalize.title') }), _jsx("p", { className: "text-sm text-red-100 mt-1", children: t('clinical.soap.cannotFinalize.subtitle') })] })] }) }), _jsxs("div", { className: "p-6 space-y-4", children: [_jsx("div", { className: "bg-red-50 border border-red-200 rounded-lg p-4", children: _jsx("p", { className: "text-sm font-medium text-red-900", children: t('clinical.soap.cannotFinalize.body') }) }), _jsx("button", { type: "button", onClick: () => setShowCPOBlockModal(false), className: "w-full bg-red-600 hover:bg-red-700 text-white py-2 rounded-lg font-medium", children: t('clinical.soap.cannotFinalize.button') })] })] }) })), _jsx(CloseInitialAssessmentConfirmModal, { isOpen: showCloseInitialConfirmModal, onClose: () => {
                    setShowCloseInitialConfirmModal(false);
                    setCloseInitialConfirmData(null);
                }, patientId: patientIdFromUrl ?? undefined, patientName: closeInitialConfirmData?.patientName, baselineId: closeInitialConfirmData?.baselineId }), _jsx("header", { className: "border-b border-slate-200 bg-white px-6 py-4", children: _jsxs("div", { className: "mx-auto max-w-6xl flex flex-col gap-2", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx("div", { className: "flex items-center gap-4", children: _jsx("div", { className: "flex flex-col", children: _jsxs("span", { className: "text-sm text-slate-900 font-medium font-apple", children: [greeting, ", ", clinicianDisplayName] }) }) }), _jsxs("button", { type: "button", onClick: handleLogout, className: "flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 hover:text-slate-900 hover:bg-slate-50 rounded-lg transition-colors font-apple", title: t('shell.common.logout'), "aria-label": t('shell.common.logout'), children: [_jsx(LogOut, { className: "w-4 h-4" }), t('shell.common.logout')] })] }), _jsxs("div", { className: "flex items-center justify-between pt-2 border-t border-slate-100", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-base font-semibold text-slate-900 font-apple", children: currentPatient
                                                ? visitType === 'initial'
                                                    ? t('shell.professionalWorkflow.initialAssessment')
                                                    : t('shell.professionalWorkflow.followUp')
                                                : t('shell.professionalWorkflow.clinicalWorkflow') }), currentPatient && (_jsxs(_Fragment, { children: [_jsx("span", { className: "text-slate-400", children: "\u2014" }), _jsx("span", { className: "text-base text-slate-700 font-apple", children: currentPatient.fullName ||
                                                        `${currentPatient.firstName || ''} ${currentPatient.lastName || ''}`.trim() ||
                                                        'Patient' })] }))] }), _jsxs("button", { type: "button", onClick: () => {
                                        const uid = user?.uid;
                                        if (uid) {
                                        ...
                                        navigate('/command-center');
                                    }, className: "flex items-center gap-2 px-3 py-1.5 text-sm text-slate-600 hover:text-primary-blue hover:bg-blue-50 rounded-lg transition-colors font-apple", children: [_jsx(ArrowLeft, { className: "w-4 h-4" }), t('shell.commandCenter.title')] })] })] }) }), _jsx("div", { className: "mx-auto max-w-6xl px-6 py-10 space-y-10", children: visitType === 'follow-up' ? (!followUpBaselineChecked ? (_jsxs("div", { className: "rounded-2xl border border-slate-200 bg-slate-50 p-8 text-center", children: [_jsx(Loader2, { className: "w-8 h-8 text-slate-400 mx-auto mb-3 animate-spin" }), _jsx("p", { className: "text-sm text-slate-600", children: "Checking baseline\u2026" })] })) : followUpBaselineChecked && !followUpClinicalState ? (_jsxs("div", { className: "rounded-2xl border-2 border-amber-200 bg-amber-50 p-8 text-center", children: [_jsx(AlertCircle, { className: "w-12 h-12 text-amber-600 mx-auto mb-4" }), _jsx("h2", { className: "text-lg font-semibold text-slate-900 mb-2", children: "No baseline \u2014 follow-up not available" }), _jsx("p", { className: "text-sm text-slate-700 mb-4 max-w-md mx-auto", children: "You cannot start a follow-up without a prior initial assessment. Complete an initial assessment for this patient first; the baseline is then built from that and is the single source of truth for follow-up." }), _jsx("div", { className: "flex flex-wrap items-center justify-center gap-3", children: _jsx("button", { type: "button", onClick: () => navigate(patientId ? `/workflow?patientId=${patientId}` : '/command-center'), className: "inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-800 text-white text-sm font-medium hover:bg-slate-700", children: "Start initial assessment" }) })] })) : (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "sticky top-4 z-30 bg-slate-50/95 border border-blue-200 rounded-lg p-6 backdrop-blur", children: [_jsx("h2", { className: "text-lg font-semibold text-slate-900 mb-4", children: "Patient context" }), _jsxs("div", { className: "grid gap-4 sm:grid-cols-2 lg:grid-cols-3", children: [_jsxs("div", { className: "bg-slate-50 border border-slate-200 rounded-xl p-4", children: [_jsx("p", { className: "text-xs uppercase tracking-wide text-slate-400 font-apple font-light mb-2", children: "Patient" }), _jsx("p", { className: "text-lg font-semibold text-slate-900 font-apple", children: currentPatient?.fullName || `${currentPatient?.firstName || ''} ${currentPatient?.lastName || ''}`.trim() || demoPatient.name }), _jsxs("div", { className: "flex items-center gap-2 mt-1", children: [_jsx("p", { className: "text-sm text-slate-500 font-apple font-light", children: currentPatient?.email || demoPatient.email }), (() => {
                                                            const dob = currentPatient?.dateOfBirth || currentPatient?.birthDate;
                                                            const age = dob ? calculateAge(dob) : null;
                                                            return age !== null ? (_jsxs("span", { className: "text-sm text-slate-500 font-apple font-light", children: ["\u00B7 ", age, " years"] })) : null;
                                                        })()] }), (patientClinicalInfo.allergies || patientClinicalInfo.contraindications) && (_jsxs("div", { className: "mt-3 pt-3 border-t border-slate-200 space-y-2", children: [patientClinicalInfo.allergies && (_jsxs("div", { className: "flex items-start gap-2", children: [_jsx(AlertCircle, { className: "w-3.5 h-3.5 text-amber-600 mt-0.5 flex-shrink-0" }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("p", { className: "text-xs font-semibold text-amber-800 font-apple", children: "Allergies" }), _jsx("p", { className: "text-xs text-amber-700 font-apple font-light mt-0.5", children: patientClinicalInfo.allergies.join(', ') })] })] })), patientClinicalInfo.contraindications && (_jsxs("div", { className: "flex items-start gap-2", children: [_jsx(AlertCircle, { className: "w-3.5 h-3.5 text-red-600 mt-0.5 flex-shrink-0" }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("p", { className: "text-xs font-semibold text-red-800 font-apple", children: "Contraindications" }), _jsxs("p", { className: "text-xs text-red-700 font-apple font-light mt-0.5", children: [patientClinicalInfo.contraindications.slice(0, 2).join('; '), patientClinicalInfo.contraindications.length > 2 && ` (+${patientClinicalInfo.contraindications.length - 2} more)`] })] })] }))] })), workflowConsentStatus?.hasValidConsent ? (_jsxs("div", { className: "mt-3 flex items-center gap-2 pt-2 border-t border-slate-200", children: [_jsx(CheckCircle, { className: "w-4 h-4 text-green-600" }), _jsx("span", { className: "text-xs text-slate-600 font-apple font-light", children: "Consent valid (ON)" })] })) : (_jsx("div", { className: "mt-3 pt-3 border-t border-slate-200", children: _jsx("div", { className: "rounded-lg border border-red-200 bg-red-50 p-3", children: _jsxs("div", { className: "flex items-start gap-2", children: [_jsx(AlertCircle, { className: "w-4 h-4 text-red-600 mt-0.5 flex-shrink-0" }), _jsx("div", { className: "flex-1", children: _jsx("p", { className: "text-xs font-semibold text-red-800 font-apple", children: "Consent Required" }) })] }) }) }))] }), _jsxs("div", { className: "bg-slate-50 border border-slate-200 rounded-xl p-4", children: [_jsxs("div", { className: "flex items-center gap-2 mb-2", children: [_jsx(FileText, { className: "w-4 h-4 text-slate-400" }), _jsx("p", { className: "text-xs uppercase tracking-wide text-slate-400 font-apple font-light", children: "Last Session" })] }), lastEncounter.loading ? (_jsx("p", { className: "text-sm text-slate-500 font-apple font-light", children: "Loading..." })) : lastEncounter.error ? (_jsx("p", { className: "text-sm text-red-600 font-apple font-light", children: "Error loading session" })) : lastEncounter.data ? (_jsxs("div", { children: [_jsx("p", { className: "text-sm font-semibold text-slate-900 font-apple", children: formatLastSessionDate(lastEncounter.data) || 'Previous session' }), lastEncounter.data.soap && (_jsx("button", { onClick: () => {
                                                            ...
                                                            }, className: "mt-2 text-xs text-blue-600 hover:text-blue-800 underline font-apple font-light", children: "View last SOAP note \u2192" }))] })) : isFirstSession === true ? (_jsx("p", { className: "text-sm text-slate-700 font-apple font-light", children: "Session 1" })) : (_jsx("p", { className: "text-sm text-slate-500 font-apple font-light", children: "No previous sessions" }))] }), _jsxs("div", { className: "bg-slate-50 border border-slate-200 rounded-xl p-4", children: [_jsx("p", { className: "text-xs uppercase tracking-wide text-slate-400 font-apple font-light mb-2", children: t('workflow.visit.visitTypeLabel') }), _jsxs("div", { className: "flex flex-col gap-1", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(CheckCircle, { className: "w-4 h-4 text-blue-600" }), _jsx("span", { className: "text-sm font-semibold text-slate-900 font-apple", children: visitType === 'follow-up' ? t('workflow.visit.followupVisit') : t('workflow.visit.initialVisit') })] }), _jsx("p", { className: "text-sm text-slate-600 font-apple font-light", children: getSessionOrdinalLabel((visitCount.data ?? 0) + 1) })] }), previousTreatmentPlan && (_jsxs("div", { className: "mt-2 flex items-center gap-2", children: [_jsx(CheckCircle, { className: "w-4 h-4 text-green-600" }), _jsx("span", { className: "text-xs text-slate-600 font-apple font-light", children: t('workflow.visit.previousPlanLoaded') })] }))] })] })] }), visitType === 'follow-up' && (inClinicItems.length > 0 || homeProgramItems.length > 0) && (_jsxs(_Fragment, { children: [inClinicItems.length > 0 && (() => {
                                    const allInClinicDone = inClinicItems.every((i) => i.completed);
                                    return (_jsxs("div", { className: "bg-white border border-blue-200 rounded-lg p-6", children: [_jsxs("div", { className: "flex items-start gap-3 mb-4", children: [_jsx("span", { className: "text-2xl", children: "\uD83D\uDDD3\uFE0F" }), _jsxs("div", { className: "flex-1", children: [_jsx("h2", { className: "text-lg font-semibold text-slate-900 mb-1", children: "Today's in-clinic treatment" }), _jsx("p", { className: "text-sm text-slate-600", children: "Confirm or adjust what was planned previously." })] }), _jsxs("div", { className: "flex items-center gap-3", children: [_jsxs("button", { type: "button", onClick: () => setInClinicItems((prev) => prev.map((i) => ({ ...i, completed: !allInClinicDone }))), className: "flex items-center gap-1.5 text-sm text-slate-500 hover:text-blue-600 transition-colors", title: allInClinicDone ? t('shell.professionalWorkflow.unmarkAll') : t('shell.professionalWorkflow.markAllDone'), children: [_jsx("input", { type: "checkbox", checked: allInClinicDone, readOnly: true, className: "w-4 h-4 text-blue-600 rounded" }), _jsx("span", { children: t('shell.professionalWorkflow.allDoneLabel') })] }), _jsxs("div", { className: "flex items-center gap-2 text-sm text-blue-600", children: [_jsx(CheckCircle, { className: "w-4 h-4" }), _jsx("span", { children: t('shell.professionalWorkflow.todayTreatmentConfirmed') })] })] })] }), _jsx(SuggestedFocusEditor, { items: inClinicItems, onChange: setInClinicItems, onFinishSession: undefined, hideHeader: true })] }));
                                })(), homeProgramItems.length > 0 && (() => {
                                    const allHEPDone = homeProgramItems.every((i) => i.completed);
                                    return (_jsx(HomeProgramBlock, { items: homeProgramItems, onChange: setHomeProgramItems, allDone: allHEPDone, onSelectAllClick: () => setHomeProgramItems((prev) => prev.map((i) => ({ ...i, completed: !allHEPDone }))) }));
                                })()] })), _jsxs("div", { className: "bg-white border border-blue-200 rounded-lg p-6", children: [visitType === 'follow-up' && (_jsx("p", { className: "text-xs text-slate-500 mb-3 font-apple font-light", children: t('workflow.visit.followupContextHint') })), _jsxs("div", { className: "flex items-start gap-3 mb-4", children: [_jsx("span", { className: "text-2xl", children: visitType === 'follow-up' ? '📝' : '🎙️' }), _jsxs("div", { className: "flex-1", children: [_jsx("h2", { className: "text-lg font-semibold text-slate-900 mb-1", children: visitType === 'follow-up' ? t('workflow.visit.followupClinicalUpdate') : t('workflow.visit.clinicalNotes') }), _jsx("p", { className: "text-sm text-slate-600", children: visitType === 'follow-up'
                                                        ? t('workflow.visit.followupNotesDesc')
                                                        : t('workflow.visit.initialNotesDesc') })] }), transcript?.trim() && (_jsxs("div", { className: "flex items-center gap-2 text-sm text-blue-600", children: [_jsx(CheckCircle, { className: "w-4 h-4" }), _jsx("span", { children: visitType === 'follow-up' ? t('workflow.visit.clinicalUpdateCaptured') : t('workflow.visit.clinicalNotesCaptured') })] }))] }), hasRestoredFromAutoSave && (_jsx("div", { className: "mb-3 rounded-md border border-yellow-300 bg-yellow-50 px-3 py-2 text-xs text-yellow-900", children: restoredFromInterrupted
                                        ? 'Sesión reanudada. El nuevo audio se añadirá al transcript anterior.'
                                        : 'Session restored from auto-save.' })), isRestoringTranscript && (_jsxs("div", { className: "mb-3 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900 flex items-center gap-2", children: [_jsx(Loader2, { className: "w-3.5 h-3.5 animate-spin" }), "Cargando transcript anterior\u2026"] })), _jsx(Suspense, { fallback: _jsx(LoadingSpinner, {}), children: _jsx(AnalysisTab, { currentPatient: currentPatient, patientIdFromUrl: patientIdFromUrl, patientClinicalInfo: patientClinicalInfo, calculateAge: calculateAge, consentStatus: consentStatus, consentPending: consentPending, consentToken: consentToken, consentLink: consentLink, smsError: smsError, user: user, setConsentStatus: setConsentStatus, setPatientHasConsent: setPatientHasConsent, setConsentPending: setConsentPending, setSmsError: setSmsError, handleCopyConsentLink: handleCopyConsentLink, handleResendConsentSMS: handleResendConsentSMS, lastEncounter: lastEncounter, isFirstSession: isFirstSession, formatLastSessionDate: formatLastSessionDate, visitType: visitType, visitCount: visitCount, sessionTypeConfig: sessionTypeConfig, previousTreatmentPlan: previousTreatmentPlan, setIsInitialPlanModalOpen: setIsInitialPlanModalOpen, physioNotes: physioNotes, setPhysioNotes: setPhysioNotes, recordingTime: recordingTime, isRecording: isRecording, startRecording: startRecording, stopRecording: stopRecording, transcript: transcript, setTranscript: setTranscript, transcriptError: transcriptError, transcriptMeta: transcriptMeta, languagePreference: languagePreference, setLanguagePreference: setLanguagePreference, mode: mode, setMode: setMode, isTranscribing: isTranscribing, isProcessing: isProcessing, audioStream: audioStream, handleAnalyzeWithVertex: handleAnalyzeWithVertex, attachments: attachments, isUploadingAttachment: isUploadingAttachment, attachmentError: attachmentError, removingAttachmentId: removingAttachmentId, handleAttachmentUpload: handleAttachmentUpload, handleAttachmentRemove: handleAttachmentRemove, niagaraResults: niagaraResults, interactiveResults: interactiveResults, selectedEntityIds: selectedEntityIds, setSelectedEntityIds: setSelectedEntityIds, continueToEvaluation: continueToEvaluation, analysisError: analysisError, successMessage: successMessage, setAnalysisError: setAnalysisErrorWithRecovery, setSuccessMessage: setSuccessMessage, onTodayFocusChange: setTodayFocus, onFinishSession: undefined, hideHeader: true, todayFocusBlockRenderedByParent: visitType === 'follow-up', resumeLoadFailed: resumeLoadFailed, selectedRedFlagIds: selectedRedFlagIds, onRedFlagSelectionChange: setSelectedRedFlagIds, redFlagDecisions: redFlagDecisions, onRedFlagDecisionChange: setRedFlagDecisions, onConfirmFollowUpRedFlags: () => {
                                            ...
                                            if (hasReferralStop) {
                                        ...
                                        }, onGenerateReferralReport: handleOpenReferralReport }) })] }), (!(followUpAlerts?.red_flags?.length) || followUpDecisionResolved) && (_jsxs("div", { className: "bg-white border border-blue-200 rounded-lg p-6", "data-section": "soap", children: [_jsxs("div", { className: "flex items-start gap-3 mb-4", children: [_jsx("span", { className: "text-2xl", children: "\uD83D\uDCDD" }), _jsxs("div", { className: "flex-1", children: [_jsx("h2", { className: "text-lg font-semibold text-slate-900 mb-1", children: "Documentation" }), _jsx("p", { className: "text-sm text-slate-600", children: "Review and finalize your SOAP note." })] }), localSoapNote && (_jsxs("div", { className: "flex items-center gap-2 text-sm text-blue-600", children: [_jsx(CheckCircle, { className: "w-4 h-4" }), _jsx("span", { children: "SOAP note generated" })] }))] }), visitType === 'follow-up' && followUpPatternInsight && (_jsxs("div", { className: "mb-6 rounded-lg border border-slate-200 bg-sky-50/50 p-4", children: [_jsx("h3", { className: "text-sm font-semibold text-slate-700 mb-1", children: "AI Patient Pattern Insight" }), _jsx("p", { className: "text-xs text-slate-500 mb-2", children: "(Not part of the medical record)" }), _jsx("p", { className: "text-sm text-slate-700", children: followUpPatternInsight.description })] })), visitType === 'follow-up' && followUpConsiderations && followUpConsiderations.length > 0 && (_jsxs("div", { className: "mb-6 rounded-lg border border-slate-200 bg-amber-50/50 p-4", children: [_jsx("h3", { className: "text-sm font-semibold text-slate-700 mb-1", children: "AI Clinical Considerations" }), _jsx("p", { className: "text-xs text-slate-500 mb-3", children: "(Not part of the medical record)" }), _jsx("ul", { className: "list-disc list-inside space-y-1 text-sm text-slate-700 mb-3", children: followUpConsiderations.map((line, i) => (_jsx("li", { children: line }, i))) }), _jsx("button", { type: "button", onClick: () => {
                                                ...
                                                setFollowUpConsiderations(null);
                                            }, className: "text-sm font-medium text-amber-800 hover:text-amber-900 underline", children: "Insert into plan" })] })), _jsx(Suspense, { fallback: _jsx(LoadingSpinner, {}), children: _jsx(SOAPTab, { localSoapNote: localSoapNote, soapStatus: soapStatus, visitType: visitType, isGeneratingSOAP: isGeneratingSOAP, patientId: patientId, sessionId: sessionId, handleGenerateSoap: visitType === 'follow-up' ? handleGenerateSOAPFollowUp : handleGenerateSoap, handleSaveSOAP: handleSaveSOAP, handleRegenerateSOAP: handleRegenerateSOAP, handleFinalizeSOAP: handleFinalizeSOAP, handleUnfinalizeSOAP: handleUnfinalizeSOAP, setIsShareMenuOpen: setIsShareMenuOpen, skipPlanValidation: Object.values(redFlagDecisions).some(d => d.decision === 'referral_stop'), workflowMetrics: workflowMetrics, workflowRoute: workflowRoute, soapTokenOptimization: soapTokenOptimization, niagaraResults: visitType === 'follow-up' ? null : niagaraResults, followUpHasContent: visitType === 'follow-up' ? Boolean(transcript?.trim() || inClinicItems.length > 0 || homeProgramItems.length > 0) : undefined, transcript: transcript, physicalExamResults: physicalExamResults, treatmentReminder: treatmentReminder, analysisError: analysisError, successMessage: successMessage, setAnalysisError: setAnalysisError, setSuccessMessage: setSuccessMessage, setVisitType: setVisitType, 
                                        // ✅ FOLLOW-UP WORKFLOW: Transcript input props for follow-up visits
                                        recordingTime: recordingTime, isRecording: isRecording, startRecording: startRecording, stopRecording: stopRecording, setTranscript: setTranscript, transcriptError: transcriptError, transcriptMeta: transcriptMeta, languagePreference: languagePreference, setLanguagePreference: setLanguagePreference, mode: mode, setMode: setMode, isTranscribing: isTranscribing, isProcessing: isProcessing, audioStream: audioStream, handleAnalyzeWithVertex: handleAnalyzeWithVertex, attachments: attachments, isUploadingAttachment: isUploadingAttachment, attachmentError: attachmentError, removingAttachmentId: removingAttachmentId, handleAttachmentUpload: handleAttachmentUpload, handleAttachmentRemove: handleAttachmentRemove, onCloseInitialAssessment: undefined, onBackToCommandCenter: () => {
                                            if (sessionId) {
                                                sessionService.updateSession(sessionId, { status: 'interrupted' }).catch(() => { });
                                            }
                                            navigate('/command-center');
                                        }, patientEmail: currentPatient?.email, patientFirstName: currentPatient?.firstName || currentPatient?.personalInfo?.firstName || '', professionalName: clinicianDisplayName || '', professionalTitle: professionalProfile?.profession || 'Fisioterapeuta', patientName: currentPatient?.fullName ?? `${currentPatient?.firstName || ''} ${currentPatient?.lastName || ''}`.trim(), redFlagDecisions: redFlagDecisions }) })] })), visitType !== 'follow-up' && ((() => {
                            ...
                            return (_jsx("div", { className: "sticky bottom-0 bg-white border-t border-blue-200 p-4 mt-6 shadow-lg z-10", children: _jsx("button", { onClick: buttonAction, disabled: buttonDisabled, className: `w-full py-3 px-6 rounded-lg font-medium transition-colors ${buttonDisabled
                                        ? 'bg-blue-300 text-blue-100 cursor-not-allowed'
                                        : 'bg-blue-600 text-white hover:bg-blue-700'}`, children: buttonDisabled ? 'Processing...' : buttonLabel }) }));
                        })())] }))) : (_jsxs(_Fragment, { children: [_jsx("nav", { className: "flex flex-wrap gap-2", children: [
                                ...
                                .map((tab) => (_jsx("button", { onClick: () => setActiveTab(tab.id), className: `rounded-full px-4 py-2 text-sm transition ${effectiveActiveTab === tab.id
                                    ? "bg-gradient-to-r from-primary-blue to-primary-purple text-white shadow font-apple"
                                    : "bg-white border border-slate-200 text-slate-600 hover:border-slate-300"}`, children: tab.label }, tab.id))) }), effectiveActiveTab === "analysis" && (_jsx(Suspense, { fallback: _jsx(LoadingSpinner, {}), children: _jsx(AnalysisTab, { currentPatient: currentPatient, patientIdFromUrl: patientIdFromUrl, patientClinicalInfo: patientClinicalInfo, calculateAge: calculateAge, consentStatus: consentStatus, consentPending: consentPending, consentToken: consentToken, consentLink: consentLink, smsError: smsError, user: user, setConsentStatus: setConsentStatus, setPatientHasConsent: setPatientHasConsent, setConsentPending: setConsentPending, setSmsError: setSmsError, handleCopyConsentLink: handleCopyConsentLink, handleResendConsentSMS: handleResendConsentSMS, lastEncounter: lastEncounter, isFirstSession: isFirstSession, formatLastSessionDate: formatLastSessionDate, visitType: visitType, visitCount: visitCount, sessionTypeConfig: sessionTypeConfig, previousTreatmentPlan: previousTreatmentPlan, setIsInitialPlanModalOpen: setIsInitialPlanModalOpen, physioNotes: physioNotes, setPhysioNotes: setPhysioNotes, recordingTime: recordingTime, isRecording: isRecording, startRecording: startRecording, stopRecording: stopRecording, transcript: transcript, setTranscript: setTranscript, transcriptError: transcriptError, transcriptMeta: transcriptMeta, languagePreference: languagePreference, setLanguagePreference: setLanguagePreference, mode: mode, setMode: setMode, isTranscribing: isTranscribing, isProcessing: isProcessing, audioStream: audioStream, handleAnalyzeWithVertex: handleAnalyzeWithVertex, attachments: attachments, isUploadingAttachment: isUploadingAttachment, attachmentError: attachmentError, removingAttachmentId: removingAttachmentId, handleAttachmentUpload: handleAttachmentUpload, handleAttachmentRemove: handleAttachmentRemove, niagaraResults: niagaraResults, interactiveResults: interactiveResults, selectedEntityIds: selectedEntityIds, setSelectedEntityIds: setSelectedEntityIds, continueToEvaluation: continueToEvaluation, analysisError: analysisError, successMessage: successMessage, setAnalysisError: setAnalysisErrorWithRecovery, setSuccessMessage: setSuccessMessage, onTodayFocusChange: setTodayFocus, onFinishSession: undefined, hideHeader: false, resumeLoadFailed: resumeLoadFailed, selectedRedFlagIds: selectedRedFlagIds, onRedFlagSelectionChange: setSelectedRedFlagIds, redFlagDecisions: redFlagDecisions, onRedFlagDecisionChange: setRedFlagDecisions, onConfirmFollowUpRedFlags: () => {
                                    const hasReferralStop = Object.values(redFlagDecisions).some((d) => d.decision === 'referral_stop');
                                    if (hasReferralStop) {
                                        handleOpenReferralReport();
                                    }
                                ...
                                }, onGenerateReferralReport: handleOpenReferralReport }, `analysis-${(followUpAlerts?.red_flags?.length ?? 0)}`) })), effectiveActiveTab === "evaluation" && (_jsx(Suspense, { fallback: _jsx(LoadingSpinner, {}), children: _jsx(EvaluationTab, { visitType: visitType, filteredEvaluationTests: filteredEvaluationTests, evaluationTests: evaluationTests, completedCount: completedCount, detectedCaseRegion: detectedCaseRegion, pendingAiSuggestions: pendingAiSuggestions, allAiSuggestions: aiSuggestions, isTestAlreadySelected: isTestAlreadySelected, addEvaluationTest: addEvaluationTest, removeEvaluationTest: removeEvaluationTest, updateEvaluationTest: updateEvaluationTest, createEntryFromLibrary: createEntryFromLibrary, createCustomEntry: createCustomEntry, customTestName: customTestName, customTestRegion: customTestRegion, customTestResult: customTestResult, customTestNotes: customTestNotes, isCustomFormOpen: isCustomFormOpen, setCustomTestName: setCustomTestName, setCustomTestRegion: setCustomTestRegion, setCustomTestResult: setCustomTestResult, setCustomTestNotes: setCustomTestNotes, setIsCustomFormOpen: setIsCustomFormOpen, resetCustomForm: resetCustomForm, handleAddCustomTest: handleAddCustomTest, handleLibrarySelect: handleLibrarySelect, handleGenerateSoap: handleGenerateSoap, sessionTypeFromUrl: sessionTypeFromUrl, workflowRoute: workflowRoute }) })), effectiveActiveTab === "soap" && (_jsx(Suspense, { fallback: _jsx(LoadingSpinner, {}), children: _jsx(SOAPTab, { localSoapNote: localSoapNote, soapStatus: soapStatus, visitType: visitType, isGeneratingSOAP: isGeneratingSOAP, patientId: patientId, sessionId: sessionId, handleGenerateSoap: visitType === 'follow-up' ? handleGenerateSOAPFollowUp : handleGenerateSoap, handleSaveSOAP: handleSaveSOAP, handleRegenerateSOAP: handleRegenerateSOAP, handleFinalizeSOAP: handleFinalizeSOAP, handleUnfinalizeSOAP: handleUnfinalizeSOAP, setIsShareMenuOpen: setIsShareMenuOpen, skipPlanValidation: Object.values(redFlagDecisions).some(d => d.decision === 'referral_stop'), workflowMetrics: workflowMetrics, workflowRoute: workflowRoute, soapTokenOptimization: soapTokenOptimization, niagaraResults: niagaraResults, transcript: transcript, physicalExamResults: physicalExamResults, treatmentReminder: treatmentReminder, analysisError: analysisError, successMessage: successMessage, setAnalysisError: setAnalysisError, setSuccessMessage: setSuccessMessage, setVisitType: setVisitType, onBackToCommandCenter: () => {
                                    if (sessionId) {
                                        sessionService.updateSession(sessionId, { status: 'interrupted' }).catch(() => { });
                                    }
                                    navigate('/command-center');
                                }, patientEmail: currentPatient?.email, patientFirstName: currentPatient?.firstName || currentPatient?.personalInfo?.firstName || '', professionalName: clinicianDisplayName || '', professionalTitle: professionalProfile?.profession || 'Fisioterapeuta', patientName: currentPatient?.fullName ?? `${currentPatient?.firstName || ''} ${currentPatient?.lastName || ''}`.trim(), redFlagDecisions: redFlagDecisions }) }))] })) }), _jsx(FeedbackWidget, {}), _jsx(ReferralReportModal, { isOpen: referralReportOpen, onClose: () => setReferralReportOpen(false), reportData: referralReportData }), currentPatient && (_jsx(InitialPlanModal, { isOpen: isInitialPlanModalOpen, onClose: () => setIsInitialPlanModalOpen(false), patientId: patientIdFromUrl || demoPatient.id, patientName: currentPatient?.fullName || `${currentPatient?.firstName || ''} ${currentPatient?.lastName || ''}`.trim() || demoPatient.name, onPlanCreated: handlePlanCreated })), (() => {
                if (!currentPatient || !user?.uid || !workflowConsentStatus)
                    return false;
                const jurisdiction = getCurrentJurisdiction();
                ...
                return currentResolution.channel !== 'none';
            })() && (_jsx(VerbalConsentModal, { isOpen: showVerbalConsentModal, onClose: () => {
                    // Don't allow closing if workflow is blocked
                    if (workflowBlocked) {
                        return; // Modal must stay open until consent is obtained
                    }
                    setShowVerbalConsentModal(false);
                }, patientId: patientIdFromUrl || demoPatient.id, patientName: currentPatient?.fullName || `${currentPatient?.firstName || ''} ${currentPatient?.lastName || ''}`.trim() || demoPatient.name, physiotherapistId: user.uid, physiotherapistName: clinicianDisplayName, onConsentObtained: async (consentId) => {
                ...
                } })), showWorkflowFeedback && workflowRoute && user?.uid && (_jsx("div", { className: "fixed bottom-4 right-4 max-w-md z-50", children: _jsx(WorkflowFeedback, { sessionId: sessionId || `${user.uid}-${sessionStartTime.getTime()}`, patientId: patientIdFromUrl || demoPatient.id, userId: user.uid, workflowType: workflowRoute.type === 'follow-up' ? 'follow-up' : 'initial', detectionConfidence: workflowRoute.auditLog.detectionResult.confidence, onClose: () => setShowWorkflowFeedback(false) }) })), localSoapNote && soapStatus === 'finalized' && user?.uid && (_jsx(UniversalShareMenu, { isOpen: isShareMenuOpen, onClose: () => setIsShareMenuOpen(false), shareOptions: {
                    noteContent: [
                        "Subjective:",
                        localSoapNote.subjective || "Not documented.",
                        "",
                    ...
                    if (result.success) {
                        setSuccessMessage(result.method === 'portal'
