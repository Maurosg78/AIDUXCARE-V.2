...
const ProfessionalWorkflowPage = () => {
  ...
  // Auto-generar SOAP cuando se navega a la pestaña
  useEffect(() => {
    if (activeTab === 'soap' && !soapNote && analysisResults && transcript) {
      handleGenerateSOAP();
    }
  }, [activeTab]);
  ...
  const handleGenerateSOAP = async () => {
    ...
    try {
      ...
      // Extract patient data from transcript for accuracy
      ...
      const patientData = extractedPatient 
        ...
        : selectedPatient; // Fallback to mock if extraction fails
      
      // Detect session type from transcript
