...
import { callVertexAI } from './vertex-ai-service-firebase';
...
Generate a comprehensive, medical-legal compliant SOAP note in JSON format.
...
Required JSON structure with specific medical content:
{
  ...
  private static parseVertexResponse(
    request: SOAPGenerationRequest
  ): EditableSOAPNote {
    try {
    console.log("[SOAP] Raw response to parse:", response);      const parsed = typeof response === "string" ? JSON.parse(response) : (response.text ? JSON.parse(response.text) : response);
      const now = new Date();
