...
## 2026-09-15 — v1.23 (TD-026: pérdida real de contenido confirmada, rescate parcial + fix)

Incidente real con paciente (Marta Santamaria, AiDux Air, 2026-09-14 ~18:18 hora Valencia): de una grabación de 35 minutos, solo el primer segmento (~4-8 min, 1.86MB) llegó a Storage. Diagnóstico con `gcloud logging read` sobre Cloud Run confirmó que el servidor transcribió ese segmento con éxito en 11s, pero la respuesta nunca se procesó del lado del cliente (`session_audio_backups` quedó en `transcriptionStatus: 'pending'` sin error) — consistente con la app suspendida en background a mitad del `await`. Como `finalizeNativeRecording` sube/transcribe segmentos en secuencia, el resto de la grabación (~27-31 min) nunca se subió y se perdió al reabrirse la app. Causa raíz: `FirebaseWhisperService.transcribe()` sin `AbortController` ni timeout, mismo patrón que TD-021 en otro call site. Hallazgo adicional durante el diagnóstico: el mecanismo de recuperación TD-014 (`getTranscriptTextForSession`) llevaba roto en este entorno por un índice compuesto de `session_audio_backups` definido en `firestore.indexes.json` pero nunca desplegado — mismo patrón exacto que TD-023, y el error se traga silenciosamente en `ProfessionalWorkflowPage.tsx:2640`.

Rescate: audio del segmento 0 descargado de Storage y re-transcrito directamente contra OpenAI con los mismos parámetros que `whisperProxy.js` → 4616 caracteres recuperados y persistidos en el documento original de `session_audio_backups`; índice desplegado (`firebase deploy --only firestore:indexes`) y confirmado `READY`; verificado end-to-end que la query real de la app ahora devuelve el texto. El resto de la grabación se da por perdido, sin vía de recuperación conocida.

Fix de causa raíz: `FirebaseWhisperService.transcribe()` ahora usa `AbortController` con timeout de 180s — un cuelgue real falla con error explícito (`failed_retryable`) en vez de quedar en `pending` para siempre. 3 tests nuevos en `FirebaseWhisperService.test.ts`. Pendiente, no resuelto por este fix: el timeout del cliente no cubre el caso de fondo (WebView suspendido por iOS — un timer tampoco dispara si el runtime está congelado); requiere rediseño de resiliencia (persistencia server-side del resultado, o retry de transcripciones `pending` al volver a foreground).
