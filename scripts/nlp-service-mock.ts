...
export class MockNLPService {
  ...
  static async processTranscript(
    ...
    console.log(
      `🧠 Procesando transcripción (RAG: ${options.useRAG ? "ON" : "OFF"})...`,
    );

    // 1. Extraer entidades clínicas
    const entitiesStart = Date.now();
    const _useRAG = options.useRAG || false;
    const entities = await this.extractClinicalEntities(transcript, _useRAG);
    const entitiesTime = Date.now() - entitiesStart;

    // 2. Generar notas SOAP
    const soapStart = Date.now();
    const soapNotes = await this.generateSOAPNotes(
      transcript,
      entities,
      _useRAG,
    );
    ...
    const ragTime = _useRAG ? Math.floor(totalTime * 0.3) : 0; // Simular tiempo RAG
    ...
    return {
      ...
      metrics: {
        ...
        requires_review:
          entities.length < 3 || (soapNotes.confidence_score || 0) < 0.7,
      },
    };
  }
...
${ragResult.citations
  .slice(0, 2)
  .map((c) => `- ${c.title} (${c.authors}, ${c.journal} ${c.year})`)
  .join("\n")}`;
          }
        } catch (error) {
...
`;
  ...
  private static extractEntitiesBasic(transcript: string): ClinicalEntity[] {
    ...
    const patterns = {
      ...
      treatment:
        /fisioterapia|ejercicios|terapia manual|movilización|estiramiento/gi,
    };
   ...
   */
  private static parseSOAPFromResponse(
    response: string,
    hasRAG: boolean,
  ): SOAPNotes {
    const sections = {
    ...
    const subjectiveMatch = response.match(
      /SUBJECTIVE:?\s*(.*?)(?=OBJECTIVE:|$)/is,
    );
    const objectiveMatch = response.match(
      /OBJECTIVE:?\s*(.*?)(?=ASSESSMENT:|$)/is,
    );
    ...
    const completeness =
      Object.values(sections).filter((s) => s.length > 10).length / 4;
   ...
   */
  private static generateBasicSOAP(
    transcript: string,
    hasRAG: boolean,
  ): SOAPNotes {
    return {
