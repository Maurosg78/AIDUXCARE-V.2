...
async function generateLongitudinalMetrics() {
  try {
    ...
    if (patientError || !patients || patients.length === 0) {
      console.error(
        "Error: No se encontró a la paciente Andrea Bultó",
        patientError,
      );
      return;
    }
    ...
    if (visitsError || !visits || visits.length < 2) {
      console.error(
        "Error: No se encontraron suficientes visitas para comparar",
        visitsError,
      );
      return;
    }
    ...
    console.log(
      `📊 Comparando visita ${currentVisit.id} (${new Date(currentVisit.date).toLocaleDateString()}) con visita ${previousVisit.id} (${new Date(previousVisit.date).toLocaleDateString()})`,
    );

    // 3. Obtener los contextos MCP de ambas visitas
    const { data: currentContextData, error: currentContextError } =
      await supabase
        .from("mcp_contexts")
        .select("context")
    ...
    const { data: previousContextData, error: previousContextError } =
      await supabase
        .from("mcp_contexts")
        .select("context")
    ...
    if (
      ...
      console.error(
        "Error: No se pudieron obtener los contextos MCP",
        currentContextError || previousContextError,
      );
      return;
    }
    ...
    const { data: currentSuggestions, error: currentSuggestionsError } =
      await supabase
        .from("ai_suggestions")
        .select("*")
        .eq("visit_id", currentVisit.id);

    const { data: previousSuggestions, error: previousSuggestionsError } =
      await supabase
        .from("ai_suggestions")
        .select("*")
        .eq("visit_id", previousVisit.id);
    ...
    if (currentSuggestionsError || previousSuggestionsError) {
      console.error(
        "Error: No se pudieron obtener las sugerencias IA",
        currentSuggestionsError || previousSuggestionsError,
      );
    }
    ...
    const suggestionsAccepted =
      currentSuggestions?.filter((s) => s.status === "accepted")?.length || 0;
    const suggestionsIntegrated =
      currentSuggestions?.filter((s) => s.status === "integrated")?.length || 0;

    // Métricas previas (para comparación)
    const previousSuggestionsGenerated = previousSuggestions?.length || 0;
    const previousSuggestionsAccepted =
      previousSuggestions?.filter((s) => s.status === "accepted")?.length || 0;
    const previousSuggestionsIntegrated =
      previousSuggestions?.filter((s) => s.status === "integrated")?.length ||
      0;
    ...
    if (audioError) {
      console.error(
        "Error: No se pudieron obtener las transcripciones de audio",
        audioError,
      );
    }
    ...
    if (audioData && audioData.length > 0) {
      ...
      audioData.forEach((transcription) => {
        try {
          ...
          const approvedItems = content.filter(
    ...
    console.log(
      `📈 Evolución del dolor: ${previousPainLevel}/10 → ${currentPainLevel}/10`,
    );
    ...
    const warningCount =
      currentSuggestions?.filter(
        (s) => s.type === "warning" && s.status !== "accepted",
      )?.length || 0;
    ...
    const timeSavedMinutes =
      suggestionsIntegrated * 2 + audioItemsValidated * 1;
    ...
    const metricsData = {
      ...
      details: JSON.stringify({
        previous_metrics: {
          ...
          field_matched: 0,
          warnings:
            previousSuggestions?.filter((s) => s.type === "warning")?.length ||
            0,
    ...
    if (insertError) {
      console.error(
        "Error: No se pudieron guardar las métricas longitudinales",
        insertError,
      );
      return;
    }
    ...
    console.log(
      `✅ Métricas longitudinales generadas y guardadas correctamente`,
    );
    console.log(
      `📊 Resumen: ${getEvolutionEmoji(clinicalEvolution)} Evolución clínica: ${clinicalEvolution}`,
    );
