...
async function main() {
  try {
    ...
    const professionalPassword = await askQuestion(
      "   Ingrese la contraseña del profesional: ",
    );
    ...
    if (signInError) {
      throw new Error(
        `Error al iniciar sesión como profesional: ${signInError.message}`,
      );
    }
    ...
    if (professionalError) {
      throw new Error(
        `Error al buscar el perfil del profesional: ${professionalError.message}`,
      );
    }
    ...
    console.log(
      `✅ Profesional encontrado: ${professionalName} (ID: ${professionalId})`,
    );

    // 3. Buscar si ya existe o crear usuario para el paciente
    console.log(`   Buscando perfil del paciente (${PATIENT_EMAIL})...`);

    const { data: existingPatientUser, error: patientUserError } =
      await supabase
        .from("user_profiles")
        .select("id, email")
    ...
    let patientUserId;
    const _patientEmail = PATIENT_EMAIL;

    if (patientUserError && patientUserError.code !== "PGRST116") {
      throw new Error(
        `Error al buscar el usuario paciente: ${patientUserError.message}`,
      );
    }

    if (!existingPatientUser) {
      console.log(
        `   No se encontró perfil para ${PATIENT_EMAIL}, se usará un ID simulado`,
      );
    ...
    const { data: existingPatients, error: existingPatientError } =
      await supabase
        .from("patients")
        .select("id, name")
    ...
    if (existingPatientError) {
      throw new Error(
        `Error al verificar pacientes existentes: ${existingPatientError.message}`,
      );
    }
    ...
    if (createPatientError) {
      throw new Error(
        `Error al crear el paciente: ${createPatientError.message}`,
      );
    }
    ...
    const initialFormContent = {
      subjective:
        "Paciente de 29 años que acude por dolor lumbo-cervical de moderada intensidad, de características mecánicas, que aumenta con los movimientos y mejora con el reposo. Refiere inicio paulatino hace aproximadamente 3 semanas coincidiendo con cambio de puesto de trabajo que requiere mantener posturas sedentes por periodos prolongados. No refiere traumatismos previos. No presenta irradiación ni parestesias. No síntomas de alarma.",
      objective:
        "Exploración física: Dolor a la palpación de apófisis espinosas L4-L5 y C5-C7. Contractura paravertebral bilateral. Limitación a la flexo-extensión cervical en grado leve. No signos radiculares. No déficits sensitivos ni motores. Reflejos osteotendinosos normales y simétricos.",
      assessment:
        "Dolor lumbo-cervical de origen mecánico sin signos de gravedad, probablemente relacionado con postura laboral inadecuada y estrés asociado al cambio de puesto.",
      ...
      notes:
        "Se recomienda a la paciente que realice pausas activas durante su jornada laboral. Se le han facilitado infografías con ejercicios específicos para realizar en el trabajo.",
    };
    ...
    if (createFormError) {
      throw new Error(
        `Error al crear el formulario clínico: ${createFormError.message}`,
      );
    }
    ...
    if (createFollowUpVisitError) {
      throw new Error(
        `Error al crear la visita de seguimiento: ${createFollowUpVisitError.message}`,
      );
    }
    ...
    const followUpFormContent = {
      subjective:
        "Paciente que acude a revisión. Refiere mejora parcial del dolor cervical, pero persistencia del dolor lumbar, especialmente al final de la jornada laboral. Ha implementado algunas de las recomendaciones ergonómicas y realiza los ejercicios diariamente.",
      objective: "",
      assessment: "",
      plan: "",
      notes: "",
    ...
    if (createFollowUpFormError) {
      throw new Error(
        `Error al crear el formulario para la visita de seguimiento: ${createFollowUpFormError.message}`,
      );
    }
    ...
    const aiSuggestionData = {
      ...
      content:
        "Considerando la persistencia del dolor lumbar a pesar de realizar ejercicios posturales, recomendaría valorar derivación a fisioterapia para tratamiento manual específico y refuerzo de ejercicios terapéuticos.",
      type: "suggestion",
      status: "pending",
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    ...
    if (createSuggestionError) {
      throw new Error(
        `Error al crear la sugerencia IA: ${createSuggestionError.message}`,
      );
    }
    ...
    if (createTranscriptionError) {
      throw new Error(
        `Error al crear la transcripción de audio: ${createTranscriptionError.message}`,
      );
    }

    console.log(
      `✅ Transcripción de audio creada con ID: ${audioTranscriptionId}`,
    );
    ...
    if (createAuditLogError) {
      throw new Error(
        `Error al crear registros de auditoría: ${createAuditLogError.message}`,
      );
    }

    console.log(
      `✅ ${auditEvents.length} eventos registrados en el log de auditoría`,
    );
    ...
    if (createMetricsError) {
      throw new Error(
        `Error al registrar métricas: ${createMetricsError.message}`,
      );
    }
    ...
    console.log(
      "4. Accede como paciente (paciente@aiduxcare.com) para ver sus datos",
    );
    console.log("-------------------------------------------------------");
  } catch (error) {
