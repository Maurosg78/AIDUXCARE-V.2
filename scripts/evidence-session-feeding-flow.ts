 ...
 *   npx tsx scripts/evidence-session-feeding-flow.ts
 *
...
/** Snapshot tal como se persiste en clinical_baselines (desde SOAP). */
function buildSnapshotFromSOAP(soap: typeof SOAP_INITIAL_FICTICIO) {
  return {
...
/**
 * Mapeo exacto que usa clinicalStateService.getBaselineSafe (snapshot → baselineSOAP).
 * Copia lógica para evidencia sin depender de Firestore.
 */
function mapSnapshotToBaselineSOAP(snapshot: ReturnType<typeof buildSnapshotFromSOAP>, baselineId: string, sourceSoapId: string, createdAt: Date) {
  return {
