...
const ProfessionalWorkflowPage = () => {
  ...
  console.log('🔍 [DEBUG] localStorageClearedRef.current:', localStorageClearedRef.current);
  ...
  if (isExplicitFollowUp && patientIdFromUrl && typeof window !== 'undefined' && !localStorageClearedRef.current) {
    try {
      ...
      console.log('🗑️ [DEBUG] localStorage BEFORE clear:', existingData ? 'Has data' : 'Empty');
      ...
      if (!existingData || !JSON.parse(existingData)?.transcript?.trim()) {
        ...
        console.log('✅ [WORKFLOW] 🗑️ EARLY CLEAR: Removed localStorage for follow-up visit (no existing transcript)');
      } else {
        console.log('⚠️ [WORKFLOW] Preserving localStorage - transcript exists, not clearing');
      }
  ...
  useEffect(() => {
    ...
    if (isExplicitFollowUp && patientId && !useEffectClearedRef.current) {
      // ✅ FIX: Only clear localStorage ONCE, not on every render
      const existingState = SessionStorage.getSession(patientId);
      const hasExistingTranscript = existingState?.transcript?.trim();
      
      if (!hasExistingTranscript) {
        console.log('[WORKFLOW] 🗑️ CLEARING localStorage for follow-up visit (useEffect) - no existing transcript');
        console.log('🗑️ [DEBUG] useEffect - Clearing key:', `aidux_${patientId}`);
        SessionStorage.clearSession(patientId);
        useEffectClearedRef.current = true; // Mark as cleared
      } else {
        console.log('[WORKFLOW] ⚠️ Preserving localStorage - user has transcript, not clearing');
        // Restore transcript if it exists
        if (existingState.transcript) {
          setTranscript(existingState.transcript);
          console.log('[WORKFLOW] ✅ Restored transcript from localStorage');
        }
        useEffectClearedRef.current = true; // Mark as processed
      }
    ...
    } else if (isExplicitFollowUp && patientId && useEffectClearedRef.current) {
      // Already processed, just restore transcript if it exists
      const existingState = SessionStorage.getSession(patientId);
      if (existingState?.transcript?.trim() && !transcript) {
        setTranscript(existingState.transcript);
        console.log('[WORKFLOW] ✅ Restored transcript from localStorage (subsequent render)');
      }
      return; // Don't restore anything else
    }
