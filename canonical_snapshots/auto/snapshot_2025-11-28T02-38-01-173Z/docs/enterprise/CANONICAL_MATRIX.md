...
### 0) Consentimiento del Paciente (Patient Consent) — PHIPA s.18
- **Objetivo:** obtener consentimiento informado del paciente para procesamiento transfronterizo mediante IA.
- **Cobertura de etapas:** Token generation → SMS delivery → Consent portal → Consent recording → Workflow access control
- **Componentes canónicos:**
  ...
  - `src/pages/ProfessionalWorkflowPage.tsx` (integration points) (CANONICAL)
- **I/O contrato:**
  - Input: `ConsentToken`
  - Output: `ConsentRecord` (ongoing | session-only | declined) → Workflow access granted/denied
- **Eventos clave:** `consent.token.generated`, `consent.sms.sent`, `consent.recorded`, `consent.authorized.manual`, `consent.declined`
- **Persistencia:** **sí** (Firestore: `patient_consent_tokens`, `patient_consents`)
- **Keep/Archive:** mantener canónicos; duplicados a `docs/_archive/consent/`
- **Ver:** `docs/enterprise/CANONICAL_CONSENT_WORKFLOW.md` para documentación completa
