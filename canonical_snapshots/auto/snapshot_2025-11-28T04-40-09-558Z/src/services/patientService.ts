...
export interface Patient {
  ...
  referralDiagnosis?: string; // Diagnóstico de derivación (cuando es referral)
  suspectedDiagnosis?: string; // Sospecha diagnóstica (cuando NO es referral)
  ...
  static async getPatientById(patientId: string): Promise<Patient | null> {
    try {
      ...
      if (patientSnap.exists()) {
        ...
        // Construct fullName from firstName/lastName if fullName doesn't exist
        ...
        return {
          ...
          referralDate: data.referralDate as string || '',
          referralReason: data.referralReason as string || '',
          referralDiagnosis: data.referralDiagnosis as string || undefined,
          suspectedDiagnosis: data.suspectedDiagnosis as string || undefined,
          insuranceProvider: data.insuranceProvider as string || '',
          insurancePolicy: data.insurancePolicy as string || '',
          insuranceGroup: data.insuranceGroup as string || '',
          copayAmount: data.copayAmount as number || 0,
