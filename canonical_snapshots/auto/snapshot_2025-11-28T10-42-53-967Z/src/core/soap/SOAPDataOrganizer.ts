...
export function validateUnifiedData(data: UnifiedClinicalData): {
  ...
  // Tab 2 validation - Skip for follow-up visits (physical tests are optional)
  if (data.visit.type !== 'follow-up') {
    if (!data.tab2.evaluationTests || data.tab2.evaluationTests.length === 0) {
      missingFields.push('at least one physical evaluation test');
    }
  } else {
    // For follow-up visits, physical tests are optional
    if (!data.tab2.evaluationTests || data.tab2.evaluationTests.length === 0) {
      warnings.push('No physical evaluation tests - follow-up visits may proceed without tests');
    }
  }
