  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/30">
      <div className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-in">
        ...
        <div className="mb-1 block text-sm font-medium text-slate-700">Patient</div>
        {!selectedPatient ? (
          <>
            <input
              value={query}
              ...
              id="patientSearch"
              className="mb-2 w-full rounded-lg border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue"
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="apptDate" className="mb-1 block text-sm font-medium text-slate-700">Date and Time</label>
            <input
              type="datetime-local"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              id="apptDate"
              className="w-full rounded-lg border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue"
            />
          </div>
          <div>
            <label htmlFor="apptDuration" className="mb-1 block text-sm font-medium text-slate-700">Duration (min)</label>
            <input
              type="number"
              min={15}
              step={15}
        ...
        <label htmlFor="apptNotes" className="mb-1 mt-4 block text-sm font-medium text-slate-700">Notes (optional)</label>
        <textarea
          rows={3}
          value={notes}
          ...
          id="apptNotes"
          className="w-full rounded-lg border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue"
          placeholder="Brief reason, indications…"
        />

        <div className="mt-6 flex items-center justify-end gap-3">
          <button onClick={onClose} className="rounded-xl border border-slate-300 px-4 py-2 hover:bg-slate-50 transition-colors">Cancel</button>
          <button disabled={!canCreate} onClick={handleCreate} className="bg-gradient-to-r from-primary-blue to-primary-purple hover:from-primary-blue-hover hover:to-primary-purple-hover text-white px-4 py-2 rounded-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-all">Create Appointment</button>
        </div>
      </div>
    </div>
  );
