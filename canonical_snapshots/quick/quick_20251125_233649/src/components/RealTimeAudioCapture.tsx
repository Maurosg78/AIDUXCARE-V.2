...
import { PatientConsentService } from '../services/patientConsentService'; // ✅ PHIPA compliance
...
interface RealTimeAudioCaptureProps {
  ...
  language?: 'es' | 'en';
  className?: string;
  patientId?: string; // ✅ PHIPA compliance: Required for consent verification
}
...
const RealTimeAudioCapture: React.FC<RealTimeAudioCaptureProps> = ({
  ...
  language = 'es',
  className = '',
  patientId // ✅ PHIPA compliance: Required for consent verification
}) => {
  ...
  const handleStartCapture = async () => {
    ...
    // ✅ PHIPA COMPLIANCE: Verify patient consent before recording
    if (patientId) {
      try {
        ...
        if (!hasConsent) {
          ...
          setCaptureStatus('error');
          console.warn('[AUDIO CAPTURE] Recording blocked: No patient consent', { patientId });
          return; // Block recording
        }
        console.log('[AUDIO CAPTURE] Consent verified, proceeding with recording', { patientId });
      } catch (error) {
