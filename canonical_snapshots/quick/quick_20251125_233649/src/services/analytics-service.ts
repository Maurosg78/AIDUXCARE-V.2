...
class AnalyticsService {
  ...
  private initializeTracking() {
    // Capturar TODA interacción (optimized with passive listeners)
    document.addEventListener('click', this.trackClick, { passive: true });
    document.addEventListener('input', this.trackInput, { passive: true });
    
    // Métricas de rendimiento
    this.trackPerformance();
    
    // Errores y reintentos
    window.addEventListener('error', this.trackError, { passive: true });
    
    // Tiempo en cada sección
    this.trackPageTime();
    
    // Flujos abandonados
    window.addEventListener('beforeunload', this.trackAbandonment, { passive: true });
  }
