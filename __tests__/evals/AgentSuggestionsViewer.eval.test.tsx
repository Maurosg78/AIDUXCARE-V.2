...
vi.mock(
  "../../src/core/dataSources/suggestionFeedbackDataSourceSupabase",
  () => ({
...
describe("AgentSuggestionsViewer EVAL", () => {
  ...
  const mockRecommendations = [
    createMockSuggestion(
      "recommendation",
      "Realizar ECG para evaluar posible cardiopatía isquémica",
      true,
    ),
    createMockSuggestion(
      "recommendation",
      "Ajustar dosis de metformina a 1000mg c/12h",
    ),
  ];
  ...
  const mockWarnings = [
    createMockSuggestion(
      "warning",
      "Paciente con HTA no controlada, considerar manejo urgente",
    ),
  ];
  ...
  const mockInfos = [
    createMockSuggestion(
      "info",
      "Considerar referir a nutricionista para manejo de dislipidemia",
    ),
  ];
  ...
  describe("Caso 1: Renderizado de sugerencias válidas", () => {
    it("debe mostrar las sugerencias agrupadas por tipo cuando se expande", async () => {
      ...
      mockSuggestions.forEach((suggestion) => {
        expect(
          screen.getByTestId(`suggestion-${suggestion.id}`),
        ).toBeInTheDocument();
      });
    });
    ...
    it("debe mostrar el contexto de origen para sugerencias que lo tienen disponible", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Paciente con HTA no controlada/),
        ).toBeInTheDocument();
        expect(
          screen.getByText(/Considerar referir a nutricionista/),
        ).toBeInTheDocument();
      });
  ...
  describe("Caso 2: Manejo de arrays de sugerencias vacíos", () => {
    it("debe mostrar un mensaje cuando no hay sugerencias", () => {
      ...
      expect(screen.getByTestId("no-suggestions-message")).toHaveTextContent(
        "No hay sugerencias disponibles",
      );
    });
  ...
  describe.skip("Caso 3: Interacción con las sugerencias", () => {
    it("debe permitir aceptar sugerencias y actualizar el contador", async () => {
      ...
      const integrarTextElement = screen.getByText(
        (content) =>
          typeof content === "string" &&
          content.toLowerCase().includes("sugerencias") &&
          content.toLowerCase().includes("aceptadas"),
      );
      expect(integrarTextElement).toBeInTheDocument();

      // Buscar botones que puedan contener 'Integrar' en su texto
      const integrateButton = screen.getByText(
        (content) =>
          typeof content === "string" &&
          content.toLowerCase().includes("integrar"),
      );
      ...
      await waitFor(() => {
        const successText = screen.getByText(
          (content) =>
            typeof content === "string" &&
            content.toLowerCase().includes("integradas"),
        );
  ...
  describe.skip("Caso 4: Integración con EMR", () => {
    ...
    it("no debe mostrar la opción de integrar hasta que se acepte alguna sugerencia", () => {
      ...
      expect(
        screen.queryByText((content) =>
          content.includes("sugerencias aceptadas listas para integrar"),
        ),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(
    ...
    it("debe deshabilitar la integración después de integrar una vez", async () => {
      ...
      expect(
        screen.getByText((content) => content.includes("han sido integradas")),
      ).toBeInTheDocument();
      ...
      expect(
        screen.queryByText(
          (content) =>
            typeof content === "string" &&
            content.toLowerCase().includes("integrar al emr"),
        ),
      ).not.toBeInTheDocument();
    });
  });
