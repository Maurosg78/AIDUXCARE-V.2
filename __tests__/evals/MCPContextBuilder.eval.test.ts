...
describe("MCPContextBuilder EVAL", () => {
  ...
  describe("Caso 1: Datos clínicos completos", () => {
    it("debe construir un contexto con estructura correcta a partir de datos completos", () => {
        ...
        data: [
          {
            ...
            type: "contextual",
            content:
              "Paciente femenina de 65 años acude a consulta por dolor torácico",
        ...
        data: [
          {
            ...
            type: "semantic",
            content:
              "El dolor torácico puede estar asociado a cardiopatía isquémica",
          },
        ],
      };
      ...
      expect(context.contextual.data[0].content).toBe(
        "Paciente femenina de 65 años acude a consulta por dolor torácico",
      );
      expect(context.persistent.data[0].content).toBe(
        "Antecedentes: Hipertensión arterial, Diabetes mellitus tipo 2",
      );
      expect(context.semantic.data[0].content).toBe(
        "El dolor torácico puede estar asociado a cardiopatía isquémica",
      );
  ...
  describe("Caso 2: Datos con campos críticos faltantes", () => {
    it("debe preservar la estructura del contexto a pesar de campos faltantes", () => {
      ...
      expect(context.semantic.data[0].content).toBe(
        "El dolor abdominal puede tener múltiples causas",
      );
    });
  });
        ...
        data: [
          {
            ...
            type: "semantic",
            content:
              "El dolor epigástrico puede estar asociado a enfermedad ácido-péptica",
          },
          {
  ...
  describe("Caso 4: Datos mínimos pero válidos", () => {
    it("debe generar un contexto con datos mínimos", () => {
      ...
      // Verificar que el contenido se preserva
      expect(context.contextual.data[0].content).toBe(
        "Paciente acude a control. Sin síntomas activos.",
      );
      expect(context.persistent.data[0].content).toBe(
