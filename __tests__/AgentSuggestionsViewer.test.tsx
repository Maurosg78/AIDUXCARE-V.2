...
} from "../src/core/services/EMRFormService";
...
(EMRFormService.insertSuggestion as jest.Mock).mockImplementation(
...
describe("AgentSuggestionsViewer", () => {
  ...
  it("debe manejar correctamente errores de red al integrar sugerencias", async () => {
    ...
    (EMRFormService.insertSuggestion as jest.Mock).mockRejectedValueOnce(
      networkError,
    );
    ...
    await waitFor(() => {
      expect(
        getByTestId(`suggestion-${mockSuggestions[0].id}`),
      ).toBeInTheDocument();
    });
  ...
  it("debe manejar correctamente sugerencias no integrables", async () => {
    ...
    await waitFor(() => {
      expect(
        getByTestId(`suggestion-${nonIntegrableSuggestion.id}`),
      ).toBeInTheDocument();
    });

    // Encontrar y hacer clic en el botón de aceptar
    const acceptButton = getByTestId(
      `accept-suggestion-${nonIntegrableSuggestion.id}`,
    );
    expect(acceptButton).toBeInTheDocument();
    await fireEvent.click(acceptButton);
  ...
  it("debe ser accesible", () => {
    ...
    expect(
      getByRole("region", { name: /sugerencias del copiloto/i }),
    ).toBeInTheDocument();
