...
} from "../../../src/core/agent/AgentExecutor";
...
import {
  ...
  LLMProvider,
  LLMResponse,
} from "../../../src/core/agent/LLMAdapter";

// Mock para sendToLLM
vi.mock("../../../src/core/agent/LLMAdapter", () => ({
  sendToLLM: vi.fn().mockImplementation((_context, _provider) => {
      ...
      suggestions: [
        {
          ...
          field: "diagnosis",
          content:
            "Considerar realizar radiografía de columna lumbar para evaluar posibles alteraciones estructurales.",
          sourceBlockId: "ctx-1",
          explanation:
            "El dolor lumbar persistente y la falta de respuesta a analgésicos sugieren la necesidad de estudios imagenológicos.",
          createdAt: new Date(),
          updatedAt: new Date(),
        },
        {
          ...
          field: "treatment",
          content:
            "Evaluar la necesidad de ajustar la dosis de enalapril considerando la hipertensión controlada.",
          sourceBlockId: "per-1",
          explanation:
            "El historial de hipertensión y el tratamiento actual sugieren la necesidad de revisar la medicación.",
          createdAt: new Date(),
          updatedAt: new Date(),
        },
      ],
  ...
  blocks: [
    {
      ...
      type: "contextual",
      content:
        "Paciente presenta dolor en la región lumbar desde hace 3 días. No responde bien a analgésicos.",
    ...
    },
    {
      ...
      type: "persistent",
      content:
        "Historial de hipertensión controlada. Tratamiento habitual con enalapril 10mg/día.",
      created_at: new Date().toISOString(),
    },
    {
      id: "sem-1",
      type: "semantic",
      content:
        "El dolor lumbar puede ser causado por distensión muscular, problemas posturales, hernia de disco o condiciones inflamatorias.",
      created_at: new Date().toISOString(),
    },
  ],
};
...
const mockAgent = {
  ...
  getSuggestionTypes: vi
    .fn()
    .mockReturnValue(["recommendation", "warning", "info"]),
} as unknown as ClinicalAgent;

describe("AgentExecutor", () => {
  const _mockParams: AgentExecutionParams = {
    context: mockContext,
    provider: "openai",
  };
  ...
  it("debería generar un prompt que contenga fragmentos del contexto original", async () => {
    ...
    expect(sendToLLM).toHaveBeenCalledWith(mockContext, _mockParams.provider);
  });
  ...
  it("debería utilizar diferentes LLMProviders según se especifique", async () => {
    ...
    for (const provider of providers) {
      const _params: AgentExecutionParams = {
        context: mockContext,
        provider,
      };
