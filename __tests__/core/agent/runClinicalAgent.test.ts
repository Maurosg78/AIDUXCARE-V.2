...
vi.mock("../../../src/core/agent/AgentContextBuilder", () => ({
  buildAgentContext: vi.fn().mockImplementation((_mcpContext) => {
...
describe("runClinicalAgent", () => {
  ...
  const _validMcpContext: MCPContext = {
    contextual: {
      source: "test",
      data: [
        {
  ...
  const _emptyMcpContext: MCPContext = {
  ...
  it("debería devolver un array vacío si el contexto es null o inválido", async () => {
    ...
    vi.mocked(AgentContextBuilder.buildAgentContext).mockResolvedValueOnce(
      null,
    );
  ...
  it("debería manejar errores internos sin lanzar excepciones", async () => {
    ...
    const mockExecutor = {
      execute: vi
        .fn()
        .mockRejectedValue(new Error("Error simulado en execute")),
      getSuggestions: vi.fn().mockReturnValue([]),
    };
    vi.mocked(AgentExecutor.AgentExecutor.create).mockResolvedValueOnce(
      mockExecutor as any,
    );
    const result = await runClinicalAgent("visit-123");
    expect(result).toEqual([]);
